...
const lineupSlots=['QB','RB1','RB2','WR1','WR2','TE','FLEX','K','DST'];
const matchupPlayers=(memberIndex,hasFinal=false)=>lineupSlots.map((slot,index)=>({slot,playerId:`qa-${memberIndex}-${index}`,playerName:`QA Player ${memberIndex}-${index}`,team:'PHI',position:members[memberIndex].roster[index].position,opponent:'DAL',isHome:true,isBye:false,points:hasFinal&&index===3?5.6:0,projectedPoints:[18.9,14.2,14.8,20.3,16.1,8.6,11.3,6.5,7.1][index]-(memberIndex*.1),projectionAvailable:true,status:hasFinal&&index===3?'Final':'Scheduled',kickoffAt:'2026-09-13T17:00:00Z',isLive:false,isFinal:hasFinal&&index===3,locked:hasFinal&&index===3}));
 ...
 for(const engine of ['chromium','webkit']){
  ...
  for(const width of engine==='chromium'?[390,320,430,1280]:[390]){
   ...
   await page.route('**/*.supabase.co/**',async route=>{
    ...
    if(path.endsWith('/ball_knower_weekly_scores'))return failScores?send({message:'Fixture score outage'},503):send([1,2].flatMap(week=>members.map((m,i)=>({league_id:leagues[0].id,member_id:m.id,week_number:week,live_points:i===0?5.6:i===9?0:80-i,projected_points:120-i,is_final:week===2,score_details:{hasProjectedTotal:i!==8,players:week===1?matchupPlayers(i,i===0):[]},updated_at:stamp}))));
   ...
   await page.getByRole('button',{name:'Manage Golden Arm League',exact:true}).click();let manage=page.getByRole('dialog',{name:'Golden Arm League',exact:true});await manage.waitFor();assert.match(await manage.innerText(),/JOIN CODE[\s\S]*BK-QATEST/);await manage.getByLabel('Custom code').fill('SUNDAY CREW');await manage.getByRole('button',{name:'Save custom code',exact:true}).click();await manage.getByText('SUNDAY-CREW',{exact:true}).waitFor();await manage.getByRole('button',{name:'Delete league',exact:true}).click();await manage.getByText('This deletes the league for all 10 members.',{exact:false}).waitFor();const permanentDelete=manage.getByRole('button',{name:'Permanently delete',exact:true});assert.equal(await permanentDelete.isDisabled(),true,'League deletion must stay disabled until the exact name is entered');await manage.getByLabel('Type league name to confirm deletion').fill('Golden Arm League');assert.equal(await permanentDelete.isEnabled(),true,'Exact league name must unlock the final delete action');await manage.getByRole('button',{name:'Cancel',exact:true}).click();await manage.getByRole('button',{name:'Close league management',exact:true}).click();
   await page.getByRole('button',{name:'My team',exact:true}).first().click();await page.getByRole('button',{name:'Matchup',exact:true}).click();const scoreboard=page.getByTestId('matchup-scoreboard');await scoreboard.waitFor();const myScore=scoreboard.locator('[data-matchup-member="m0"]');await myScore.locator('[data-matchup-current]').filter({hasText:'5.60'}).waitFor();assert.equal(await myScore.locator('[data-matchup-current]').innerText(),'5.60',`${engine}-${width}: earned points must occupy the primary score position after a player finishes`);assert.equal(await myScore.locator('[data-matchup-projected]').innerText(),'120.00 PROJ',`${engine}-${width}: projected total must remain secondary`);assert.match(await scoreboard.innerText(),/PROJECTED WIN CHANCE/);assert.equal(await page.locator('html').evaluate(node=>node.scrollWidth<=innerWidth+1),true,`${engine}-${width}: matchup comparison overflows horizontally`);await page.screenshot({path:`${out}/${engine}-${width}-matchup.png`,fullPage:true});await go('Fantasy');await page.locator('.bk-hq-premium').waitFor();
