...
assert.ok(/persistedRegularSeasonWeeks\s*\|\|/.test(postDraft)&&/persistedRegularSeasonWeeks\s*\|\|/.test(advancedSettings),'persisted legacy schedules must retain their existing regular-season endpoint');
...
assert.ok(/disabled=\{\s*!isCommissioner\s*\|\|\s*scoringLocked\s*\}/.test(postDraft),'the duplicate post-draft scoring control must share the scoring lock');
...
assert.ok(essentials.includes('if(isCloudConfigured)void fetchFantasyCommunications')&&essentials.includes('isCloudConfigured&&<div className="rounded-xl border border-white/10 p-3"><div className="text-[9px] font-black uppercase text-[#D4AF37]">Trade Thread'),'legacy cloud trade-thread fetching and controls must stay hidden in local leagues');
...
assert.ok(/onClick=\{sendMessage\}[\s\S]{0,120}className="min-h-11 rounded-xl/.test(postDraft),'league chat Send must keep a practical mobile touch target');
