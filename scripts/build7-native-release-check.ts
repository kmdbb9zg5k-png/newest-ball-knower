...
import { kickoffIso, normalizeTank01Odds } from '../server/nflPredictionFeed.js';
...
run('kickoff',()=>{
  ...
  assert.equal(kickoffIso('20260910','8:20 PM'),'2026-09-10T20:20:00-04:00','Tank01 compact dates must normalize to iOS-safe ISO');
...
run('ui',()=>{
  ...
  assert.match(endpoint,/linesAvailable/);
...
run('feed',()=>{
  const line=normalizeTank01Odds({
    gameID:'DAL@PHI_20260910',awayTeam:'DAL',homeTeam:'PHI',
    FanDuel:{awayTeamSpread:'3.5',homeTeamSpread:'-3.5',totalOver:'47.5',totalUnder:'47.5'},
  });
  assert.deepEqual(line,{awaySpread:3.5,homeSpread:-3.5,total:47.5,source:'FanDuel'});
  ...
  assert.match(feed,/\/getNFLBettingOdds/,'Tank01 fallback must request actual betting lines, not schedule only');
  assert.match(feed,/awayTeamSpread/);
  assert.match(feed,/totalOver/);
});
