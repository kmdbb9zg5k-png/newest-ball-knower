...
with sync_playwright() as p:
 ...
 page.click('#pause');assert page.locator('#paused').is_visible();poseBefore=[x['pose'] for x in diag(page)['players']];page.evaluate('window.bk3dTest.step(.5)');assert [x['pose'] for x in diag(page)['players']]==poseBefore;page.click('#restart');assert diag(page)['phase']=='pre';assert diag(page)['drive']['plays']==0
 ...
 result={'viewport':[w,h],'checks':'22 players, WebGL no error, <50 draw calls, passing movement, QB framing, receiver throw, run handoff + movement, pause/restart, sack/reset, orientation pause, paused pose freeze; no outbound requests','errors':errors,'outboundRequests':[u for u in requests if u.startswith(('http:','https:'))],'drawCalls':d['drawCalls'],'renderer':'Chromium software WebGL2 (SwiftShader); not real-iPhone certification'}
