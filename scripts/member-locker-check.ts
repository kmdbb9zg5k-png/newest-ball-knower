...
const lockerArt = read('ProfileLockerArt.tsx');
...
assert.match(locker, /aria-label=\{`Expand \$\{name\}'s profile photo`\}/);
assert.match(locker, /<ExpandedProfilePhoto name=\{name\}/);
assert.match(locker, /aria-label=\{`\$\{name\}'s profile photo`\}/);
assert.match(lockerArt, /locker-manager-face\.webp/);
assert.match(lockerArt, /<image href=\{LOCKER_MANAGER_FACE\}/);
