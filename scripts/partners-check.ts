...
assert.ok(data.includes("partnerType: 'Sports data supplied by Tank01'") && data.includes("category: 'data'"), 'Tank01 must be categorized as the named sports data supplier without an unsupported official-partner claim');
...
assert.match(home,/homePartners\.length\s*>\s*1\s*&&\s*<button/, 'the Home screen must offer the full directory when multiple featured partners are present');
...
console.log('Partner checks passed: exact media partner data, appropriately described data supplier, local logo, safe external link, and generic mobile-safe surfaces.');
