...
console.log(JSON.stringify({players:SOLO_PLAYERS_DATABASE.length,teamAssignments,uniqueSeeds:seeds.size,appearanceCombinations:appearances.size,approvedLeagueIdentities:19,approvedMyPlayerPresets:4,eliSource:{portrait:eliManifest.sourcePortrait,fullBody:eliManifest.sourceFullBody},productionRenderer:'responsive images; no canvas',visualFidelityApprovedFor:['bk-001-eli-rodriguez',...qa.players.map((player:{id:string})=>player.id)],fullCatalogVisualReleaseApproved:false},null,2));
