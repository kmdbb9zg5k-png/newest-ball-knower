...
for (const team of SOLO_TEAM_THEMES) {
  const standardRoster = buildSoloTeamRoster(team.abbr);
  const shapeErrors = validateRosterShape(standardRoster);
  check(standardRoster.length === TOTAL_ROSTER_SIZE, `${team.abbr}: simulated franchise roster builds ${standardRoster.length}/${TOTAL_ROSTER_SIZE}.`);
  check(shapeErrors.length === 0, `${team.abbr}: simulated roster shape invalid: ${shapeErrors.join(' ')}`);
}

const franchiseUserRoster = buildSoloTeamRoster('ABQ');
const franchisePartnerRoster = buildSoloTeamRoster('ANC');
...
const legalTrade = buildFranchiseTradeResult(franchiseUserRoster, 'ABQ', franchisePartnerRoster, 'ANC', partnerQuarterback.id, [userQuarterback.id]);
...
check(acquiredQuarterback.team === 'ABQ' && acquiredQuarterback.teamId === 'ABQ' && acquiredQuarterback.teamAbbreviation === 'ABQ' && acquiredQuarterback.teamName === 'Albuquerque Scorpions', 'A traded simulated player retained stale team identity fields.');
check(buildFranchiseTradeResult(franchiseUserRoster, 'ABQ', franchisePartnerRoster, 'ANC', partnerQuarterback.id, []).errors.length > 0, 'A pick-only Franchise trade was accepted.');
check(buildFranchiseTradeResult(franchiseUserRoster, 'ABQ', franchisePartnerRoster, 'ANC', partnerQuarterback.id, franchiseUserRoster.slice(0, 2).map(player => player.id)).errors.length > 0, 'A two-for-one Franchise trade was accepted.');
...
const testRookies = franchiseUserRoster.slice(0, 7).map((player, index) => assignPlayerToFranchiseTeam({ ...player, id: `integrity-rookie-${index}`, playerId: `integrity-rookie-${index}`, name: rookieClass2027[index].name, fullName: rookieClass2027[index].name }, 'ABQ'));
...
for (const [group, requiredPerTeam] of Object.entries(FANTASY_ROSTER_REQUIREMENTS)) {
  const available = SOLO_PLAYERS_DATABASE.filter(player => getDraftPositionGroup(player) === group).length;
