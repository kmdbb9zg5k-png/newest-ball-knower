...
def compile_study(folder):
    ...
    def material(source,face):
        ...
        # Opaque helper-cloth surfaces cover the complete trunk and pelvis.
        if exposed or y>7.0 or y< -7.25:return None
        if y>1.15:return 'jersey'
        return 'pants' if y> -3.35 else 'socks'
    for source in ['body','helper-tights']:
        ...
        if source=='helper-tights':
            ...
            coords[:,0]*=1+.045*pad;coords[:,2]+=np.sign(rest[:,2]-.015)*.013*pad
        ...
        for f in faces:
            key=material(source,f)
            if key is None:continue
            mesh=output[key];indices=[]
            for index,tex in f:
                identity=(source,index,tex)
                if identity not in lookup[key]:
                    lookup[key][identity]=len(mesh['v'])//8
                    mesh['v'].extend([*np.round(coords[index],6),*np.round(normals[index],6),*np.round(uv[tex],6)])
