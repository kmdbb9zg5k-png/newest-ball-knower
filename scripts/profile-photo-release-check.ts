...
import './profile-photo-crop-save-check';
...
assert.match(jpegMigration, /allowed_mime_types=array\['image\/webp','image\/jpeg'\]/);
assert.match(jpegMigration, /lower\(storage\.extension\(name\)\) in \('webp','jpg'\)/g);
...
assert.match(editor, /image\.style\.imageOrientation = 'from-image'/);
assert.match(editor, /image\.onerror/);
assert.match(editor, /That photo could not be opened\. Try another image\./);
