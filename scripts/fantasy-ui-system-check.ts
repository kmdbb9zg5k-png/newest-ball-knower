...
assert.ok(styles.includes('top: calc(72px + env(safe-area-inset-top))') && styles.includes('padding-bottom: calc(6rem + env(safe-area-inset-bottom))'), 'fantasy screens must reserve both iPhone safe areas and fixed app bars');
...
assert.ok(league.includes('grid-cols-[minmax(0,1fr)_56px_minmax(0,1fr)]'), 'matchup rows must protect the FLEX/WRT label from projection overlap');
