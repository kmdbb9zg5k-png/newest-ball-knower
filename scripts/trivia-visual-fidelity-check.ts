...
for (const selector of ['.bk-gauntlet-frame', '.bk-gauntlet-hero', '.bk-gauntlet-stats', '.bk-gauntlet-board', '.bk-gauntlet-daily-card', '.bk-gauntlet-tier-sheet']) {
...
assert.ok(!hub.includes('Same five') && !styles.includes('.bk-gauntlet-mantra'), 'The empty mantra cell must be replaced by the real daily challenge.');
