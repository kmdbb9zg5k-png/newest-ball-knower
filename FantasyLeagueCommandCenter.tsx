...
import {
  ...
  Settings, Shield, Sparkles, Trophy, Unlock, UserMinus, UserPlus, Users, XCircle,
} from 'lucide-react';
import { League } from './types';
import { useBallKnower } from './BallKnowerContext';
import {
  archiveSeasonResult, fetchLeagueEvents, fetchMyNotifications, fetchRosterRevisions,
  fetchSeasonArchive, LeagueEvent, LeagueNotification, markNotificationRead,
...
export const FantasyLeagueCommandCenter: React.FC<Props> = ({league,onGoToDraft,onGoToSimulation}) => {
  const {
    ...
    resetLeagueSimulation, updateSalaryCap, updateLeagueSettings, showToast,
  } = useBallKnower();
  ...
  const preflight=useMemo(()=>league.members.map(member=>{
    ...
    const complete=roster.length===20;
    const capOk=spent<=league.salaryCap;
  ...
  }),[league.members,league.salaryCap]);
  ...
  const biggestUpset=useMemo(()=>{
    ...
    const ratings=new Map(result.standings.map(s=>[s.memberId,s.teamRating]));
  ...
  const fillOpenSpots=()=>run('cpu-fill',async()=>{
    const filled=await autoFillLeagueWithAi(league.id);
    if(!filled&&openSlots>0) throw new Error('The open spots could not be filled. Refresh the league and try again.');
  });
  ...
  return <div className="min-h-[calc(100dvh-7rem)] bg-[#07090c] px-3 py-5 text-white sm:px-6">
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {tab==='overview' && <div className="grid gap-5 lg:grid-cols-[1.5fr_.9fr]">
        <section className="space-y-4"><PanelTitle icon={<Users className="h-4 w-4"/>} title="Owner Room" subtitle="Every roster, lock state and audit trail at a glance"/>
          <div className="space-y-2">{league.members.map(member=>{const mine=member.userId===currentUser?.id;const roster=member.roster||[];const spent=roster.reduce((s,p)=>s+(Number(p.salary)||0),0);return <div key={member.id} className={`rounded-2xl border p-4 ${mine?'border-[#D4AF37]/35 bg-[#D4AF37]/5':'border-white/10 bg-[#101318]'}`}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-black uppercase">{member.userName}</span>{member.isCommissioner&&<Badge>COMMISH</Badge>}{member.isAi&&<Badge>AI GM</Badge>}{mine&&<Badge>YOU</Badge>}</div><div className="mt-1 text-[11px] font-semibold text-zinc-500">{roster.length}/20 players · ${spent.toFixed(1)}M · {revisionCountByMember[member.id]||0} saved revision{revisionCountByMember[member.id]===1?'':'s'}</div></div><Status ready={member.status==='ready'}/></div></div>})}</div>
          <div className="flex flex-col gap-2 sm:flex-row"><button onClick={onGoToDraft} disabled={Boolean(operational.paused)||Boolean(operational.rostersLocked&&myMember?.status!=='ready')} className="min-h-12 flex-1 rounded-xl bg-[#D4AF37] px-4 text-xs font-black uppercase tracking-wider text-black disabled:opacity-40">{myMember?.status==='ready'?'View Locked Roster':'Build Your 20-Man Roster'}</button>{result&&<button onClick={onGoToSimulation} className="min-h-12 flex-1 rounded-xl border border-[#D4AF37]/35 px-4 text-xs font-black uppercase text-[#D4AF37]">Open Full Results</button>}</div>
        </section>
        <aside className="space-y-4"><PanelTitle icon={<ClipboardCheck className="h-4 w-4"/>} title="Pre-Sim Review" subtitle="No season starts with an invalid roster"/><div className="rounded-2xl border border-white/10 bg-[#101318] p-4"><div className="space-y-2">{preflight.map(x=><div key={x.member.id} className="flex items-center justify-between gap-3 rounded-xl bg-black/30 p-3"><div className="min-w-0"><div className="truncate text-xs font-black uppercase">{x.member.userName}</div><div className="mt-1 text-[10px] text-zinc-500">{x.rosterCount}/20 · ${x.spent.toFixed(1)}M</div></div>{x.valid?<CheckCircle2 className="h-5 w-5 text-emerald-400"/>:<XCircle className="h-5 w-5 text-red-400"/>}</div>)}</div>{operational.paused&&<Warning text="League is paused."/>}{isPublicLeague&&openSlots>0&&<Warning text={`Fill the remaining ${openSlots} public league spot${openSlots===1?'':'s'} with real players or CPU teams before starting.`}/>} {!allReady&&<Warning text={`${league.members.length-readyCount} owner(s) still need to submit.`}/>}<button onClick={launchSeason} disabled={!isCommissioner||!preflightPass||busy==='simulate'} className="mt-4 min-h-12 w-full rounded-xl bg-[#D4AF37] text-xs font-black uppercase tracking-wider text-black disabled:opacity-35"><Play className="mr-2 inline h-4 w-4"/>Start {league.settings?.seasonGames||17}-Game Season</button></div>
          <div className="rounded-2xl border border-white/10 bg-[#101318] p-4"><div className="text-[10px] font-black uppercase tracking-widest text-zinc-500">League State</div><div className="mt-3 grid grid-cols-3 gap-2"><StateChip label="League" active={!operational.paused} activeText="LIVE" inactiveText="PAUSED"/><StateChip label="Rosters" active={!operational.rostersLocked} activeText="OPEN" inactiveText="LOCKED"/><StateChip label="Invites" active={operational.inviteEnabled!==false} activeText="ON" inactiveText="OFF"/></div></div>
        </aside>
      </div>}
