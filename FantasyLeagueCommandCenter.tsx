...
export const FantasyLeagueCommandCenter: React.FC<Props> = ({league,onGoToDraft,onGoToSimulation}) => {
  ...
  }),[league.members,league.salaryCap,liveDraftComplete]);
  ...
  return <div className="min-h-[calc(100dvh-7rem)] bg-[#07090c] px-3 py-5 text-white sm:px-6">
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {tab==='overview' && <div className="space-y-4">
        <section className="rounded-2xl border border-[#D4AF37]/25 bg-[#D4AF37]/[.05] p-4"><div className="flex items-start gap-3"><Trophy className="mt-0.5 h-5 w-5 shrink-0 text-[#D4AF37]"/><div><div className="text-xs font-black uppercase text-[#D4AF37]">{liveDraftComplete?'League Draft Complete':'Draft Order Game'}</div><p className="mt-1 text-xs leading-5 text-zinc-300">{liveDraftComplete?`All ${league.liveDraft?.pickIndex||0} picks are locked and every 20-player roster is saved. The commissioner can start the ${league.settings?.seasonGames||17}-game season now.`:`Each manager builds a legal 20-player roster under the same cap. When every roster is submitted, Ball Knower simulates ${league.settings?.seasonGames||17} games. Final results determine the official fantasy draft order, with the champion receiving Pick #1.`}</p></div></div></section>
