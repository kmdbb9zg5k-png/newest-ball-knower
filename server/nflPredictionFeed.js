...
const kickoffIso=(date,time)=>{if(!date)return null;if(!time)return String(date);const clean=String(time).trim();if(/Z$|[+-]\d{2}:?\d{2}$/.test(clean))return`${date}T${clean}`;return`${date}T${clean}${easternOffsetForDate(String(date))}`};
...
export async function fetchCanonicalPredictionGames(){
  ...
  for(const timeoutMs of attempts){
    try{
      ...
      const payload=await response.json();const rows=Array.isArray(payload?.data)?payload.data:Array.isArray(payload)?payload:[];
