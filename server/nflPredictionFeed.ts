...
export type CanonicalPredictionGame={id:string;kickoffAt:string|null;away:string;home:string;awayAbbr:string|null;homeAbbr:string|null;awaySpread:number|null;homeSpread:number|null;total:number|null;awayScore:number|null;homeScore:number|null;final:boolean};
  ...
  for(const timeoutMs of attempts){
    try{
      ...
      return rows.map((g:any,i:number)=>{
        const awayAbbr=abbr(g?.away_team||g?.away);const homeAbbr=abbr(g?.home_team||g?.home);const spread=numberOrNull(g?.spread_line??g?.spread);const awayScore=numberOrNull(g?.away_score);const homeScore=numberOrNull(g?.home_score);const date=g?.gameday||g?.game_date||g?.date||null;const time=g?.gametime||g?.game_time||null;
        return{id:String(g?.game_id||g?.id||i),kickoffAt:kickoffIso(date,time),away:awayAbbr?(TEAM_NAMES[awayAbbr]||awayAbbr):'Away',home:homeAbbr?(TEAM_NAMES[homeAbbr]||homeAbbr):'Home',awayAbbr,homeAbbr,awaySpread:spread===null?null:spread>0?spread:spread<0?-Math.abs(spread):0,homeSpread:spread===null?null:spread>0?-spread:spread<0?Math.abs(spread):0,total:numberOrNull(g?.total_line??g?.over_under??g?.total),awayScore,homeScore,final:awayScore!==null&&homeScore!==null};
      });
