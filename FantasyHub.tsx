...
import { ArrowRight, Shield, Trophy, Users, Crown, Search, Star, X, Globe2, LoaderCircle, AlertTriangle, TrendingDown, TrendingUp } from 'lucide-react';
...
export const FantasyHub: React.FC<FantasyHubProps> = ({ view, onViewChange, onOpenCreateLeague, onOpenJoinLeague, onSelectLeague }) => {
  ...
  const [visibleRankingCount,setVisibleRankingCount]=useState(RANKINGS_PAGE_SIZE);
  ...
  const ranked=useMemo(()=>{
    return fantasyRankings
      ...
      .filter(player=>`${player.player_name} ${player.team} ${player.position}`.toLowerCase().includes(search.toLowerCase()));
  },[fantasyRankings,position,search]);
  const visibleRanked=useMemo(()=>ranked.slice(0,visibleRankingCount),[ranked,visibleRankingCount]);
  const hasMoreRankings=visibleRanked.length<ranked.length;
  const selectedPlayer=fantasyRankings.find(player=>player.player_key===selectedPlayerId);
  useEffect(()=>{
    let active=true;
  ...
  useEffect(()=>{setVisibleRankingCount(RANKINGS_PAGE_SIZE)},[position,search,view]);
  useEffect(()=>{
    let active=true;
    setRankingsBusy(true);
    void loadFantasyRankings().then(rows=>{
      if(!active)return;
      setFantasyRankings(rows);
      setRankingsError(rows.length?'': 'No 2026 PPR rankings have been published yet.');
    }).catch(error=>{
      if(!active)return;
      setRankingsError(error instanceof Error?error.message:'Could not load fantasy rankings.');
  ...
  return (
    <div className="min-h-[calc(100dvh-7rem)] px-4 pb-10 pt-5 text-white sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        {view==='cheatsheet'&&<section className="rounded-[2rem] border border-[#D4AF37]/35 bg-[#0b0e12] p-4 sm:p-6">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-[.25em] text-[#D4AF37]">2026 draft rankings · Full PPR</div><h2 className="mt-1 font-display text-4xl font-black uppercase">Player Cheat Sheet</h2><p className="mt-2 max-w-2xl text-sm text-zinc-400">Draft rank, 2026 projected points, 2025 actual points, and the football case behind every move.</p></div><div className="rounded-full border border-white/10 px-3 py-2 text-[10px] font-black text-zinc-400">{watchlist.length} MY GUYS</div></div>
          <div className="sticky top-[7rem] z-20 mt-4 space-y-2 border-y border-white/10 bg-black/90 py-3 backdrop-blur"><label className="flex min-h-11 items-center gap-2 rounded-xl border border-white/10 bg-[#111] px-3"><Search className="h-4 w-4 text-zinc-500"/><input value={search} onChange={event=>setSearch(event.target.value)} placeholder="Search player, team or position" className="w-full bg-transparent text-sm outline-none"/></label><div className="flex gap-2 overflow-x-auto">{['ALL',...fantasyPositions].map(item=><button key={item} onClick={()=>setPosition(item)} className={`min-h-10 shrink-0 rounded-full px-4 text-[10px] font-black ${position===item?'bg-[#D4AF37] text-black':'border border-white/10 text-zinc-400'}`}>{item}</button>)}</div><div className="px-1 text-[9px] font-black uppercase tracking-wider text-zinc-600">Showing {Math.min(visibleRankingCount,ranked.length)} of {ranked.length} players</div></div>
          {rankingsBusy?<div className="grid min-h-48 place-items-center"><LoaderCircle className="h-7 w-7 animate-spin text-[#D4AF37]"/></div>:rankingsError?<div className="mt-5 rounded-2xl border border-red-400/25 bg-red-400/5 p-5 text-red-200"><AlertTriangle className="h-6 w-6"/><h3 className="mt-3 font-black uppercase">Rankings unavailable</h3><p className="mt-2 text-xs leading-5 text-red-100/70">{rankingsError}</p></div>:<><div className="mt-3 divide-y divide-white/5">{visibleRanked.map(player=>{const up=player.point_change>=0;return <article key={player.player_key} className="py-4"><div className="grid grid-cols-[2.2rem_minmax(0,1fr)_auto] items-center gap-3"><div className="text-center text-base font-black text-zinc-500">{player.overall_rank}</div><button onClick={()=>setSelectedPlayerId(player.player_key)} className="min-w-0 text-left"><div className="truncate text-sm font-black sm:text-base">{player.player_name}</div><div className="mt-0.5 text-[10px] font-bold text-zinc-500">{player.position}{player.position_rank} · {player.team}</div></button><button onClick={()=>toggleWatch(player.player_key)} className="grid h-11 w-11 place-items-center rounded-xl border border-white/10" aria-label={`${watchlist.includes(player.player_key)?'Remove':'Add'} ${player.player_name} ${watchlist.includes(player.player_key)?'from':'to'} My Guys`}><Star className={`h-4 w-4 ${watchlist.includes(player.player_key)?'fill-[#D4AF37] text-[#D4AF37]':'text-zinc-600'}`}/></button></div><button onClick={()=>setSelectedPlayerId(player.player_key)} className="mt-3 grid w-full grid-cols-3 gap-2 text-left"><div className="rounded-xl bg-white/[.04] p-2.5"><div className="text-[8px] font-black uppercase text-zinc-600">2026 projected</div><div className="mt-1 text-base font-black">{player.projected_points_2026.toFixed(1)}</div></div><div className="rounded-xl bg-white/[.04] p-2.5"><div className="text-[8px] font-black uppercase text-zinc-600">2025 actual</div><div className="mt-1 text-base font-black text-zinc-300">{player.actual_points_2025.toFixed(1)}</div></div><div className={`rounded-xl p-2.5 ${up?'bg-emerald-400/10 text-emerald-300':'bg-red-400/10 text-red-300'}`}><div className="text-[8px] font-black uppercase opacity-70">Change</div><div className="mt-1 flex items-center gap-1 text-base font-black">{up?<TrendingUp className="h-4 w-4"/>:<TrendingDown className="h-4 w-4"/>}{up?'+':''}{player.point_change.toFixed(1)}</div></div></button><p className="mt-3 line-clamp-2 text-xs font-medium leading-5 text-zinc-400">{player.projection_reason}</p></article>})}</div>{hasMoreRankings&&<button onClick={()=>setVisibleRankingCount(count=>count+RANKINGS_PAGE_SIZE)} className="mt-4 min-h-12 w-full rounded-xl border border-[#D4AF37]/35 bg-[#D4AF37]/10 px-4 text-xs font-black uppercase tracking-wider text-[#D4AF37]">Load {Math.min(RANKINGS_PAGE_SIZE,ranked.length-visibleRanked.length)} More Players</button>}</>}
          <p className="mt-5 border-t border-white/10 pt-4 text-[10px] leading-5 text-zinc-600">2025 actuals use full-PPR scoring from the credited final-results source. 2026 values are preseason projections, not guarantees. Tap a player for the full rationale and sources.</p>
        </section>}
        {selectedPlayer&&<ModalPortal><div role="dialog" aria-modal="true" aria-label={`${selectedPlayer.player_name} projection details`} className="fixed inset-0 z-[9999] grid place-items-center overflow-y-auto overscroll-contain bg-black/80 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))] [-webkit-overflow-scrolling:touch]"><section className="my-auto w-full max-w-lg rounded-[2rem] border border-[#D4AF37]/25 bg-[#0c1016] p-5"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-black text-[#D4AF37]">OVERALL #{selectedPlayer.overall_rank} · {selectedPlayer.position}{selectedPlayer.position_rank} · {selectedPlayer.team}</div><h3 className="mt-1 text-3xl font-black">{selectedPlayer.player_name}</h3></div><button onClick={()=>setSelectedPlayerId(null)} className="grid h-11 w-11 place-items-center rounded-full border border-white/10"><X/></button></div><div className="mt-5 grid grid-cols-3 gap-2">{[['2026 projected',selectedPlayer.projected_points_2026.toFixed(1)],['2025 actual',selectedPlayer.actual_points_2025.toFixed(1)],['Difference',`${selectedPlayer.point_change>=0?'+':''}${selectedPlayer.point_change.toFixed(1)}`]].map(([label,value],index)=><div key={label} className={`rounded-xl p-3 ${index===2?(selectedPlayer.point_change>=0?'bg-emerald-400/10 text-emerald-300':'bg-red-400/10 text-red-300'):'bg-white/[.04]'}`}><div className="text-[8px] font-black uppercase opacity-60">{label}</div><div className="mt-1 text-sm font-black">{value}</div></div>)}</div><div className="mt-3 rounded-xl border border-white/10 bg-white/[.03] p-4"><div className="text-[9px] font-black uppercase tracking-widest text-[#D4AF37]">Why the projection moves</div><p className="mt-2 text-sm leading-6 text-zinc-300">{selectedPlayer.projection_reason}</p></div><div className="mt-3 text-[10px] leading-5 text-zinc-500"><a className="text-zinc-300 underline" href={selectedPlayer.actual_source_url} target="_blank" rel="noreferrer">2025 actual: {selectedPlayer.actual_source_name}</a><br/>{selectedPlayer.projection_source_url?<a className="text-zinc-300 underline" href={selectedPlayer.projection_source_url} target="_blank" rel="noreferrer">Projection reference: {selectedPlayer.projection_source_name}</a>:<>Projection reference: {selectedPlayer.projection_source_name}</>}<br/>{selectedPlayer.projection_model} · Updated {new Date(selectedPlayer.updated_at).toLocaleDateString()}</div><button onClick={()=>toggleWatch(selectedPlayer.player_key)} className="mt-4 min-h-12 w-full rounded-xl bg-[#D4AF37] text-sm font-black text-black">{watchlist.includes(selectedPlayer.player_key)?'REMOVE FROM MY GUYS':'ADD TO MY GUYS'}</button></section></div></ModalPortal>}
      </div>
    </div>
  );
};
