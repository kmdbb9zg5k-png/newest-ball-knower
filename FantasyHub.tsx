...
import { ArrowRight, Shield, Trophy, Users, Crown, CalendarDays, BarChart3, MessageSquare, MoreHorizontal } from 'lucide-react';
...
export const FantasyHub: React.FC<FantasyHubProps> = ({ onOpenCreateLeague, onOpenJoinLeague, onSelectLeague }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-7rem)] px-4 pb-10 pt-5 text-white sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        {leagues.length === 0 ? <div className="rounded-2xl border border-white/10 bg-[#101216]/90 p-10 text-center"><Trophy className="mx-auto mb-3 h-10 w-10 text-[#D4AF37]/60"/><div className="font-black uppercase">Your trophy case is empty</div><p className="mx-auto mt-2 max-w-sm text-sm text-zinc-500">Create your first league or join your friends with a code.</p></div> : <div className="grid gap-4">{leagues.map(league => {
          ...
          return <article key={league.id} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0d1014]/95 p-5 shadow-xl sm:p-6"><div className="pointer-events-none absolute right-0 top-0 h-full w-1/2 bg-[radial-gradient(circle_at_80%_45%,rgba(212,175,55,.12),transparent_55%)]"/><div className="relative z-10"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#D4AF37]">{league.commissionerId === currentUser?.id && <><Crown className="h-3.5 w-3.5"/> Commissioner · </>}{league.code}</div><h4 className="mt-2 font-display text-2xl font-black uppercase sm:text-3xl">{league.name}</h4><div className="mt-1 text-xs font-bold uppercase tracking-wider text-zinc-500">{league.members.length}/{league.maxMembers} members · {completed ? 'Complete' : 'Private league'}</div></div><button className="text-zinc-500"><MoreHorizontal/></button></div>
          <div className="mt-5 grid grid-cols-2 gap-2 border-y border-white/5 py-4 sm:grid-cols-4"><div><div className="text-[9px] font-black uppercase tracking-widest text-zinc-600">Current Phase</div><div className="mt-1 text-sm font-black text-[#D4AF37]">{completed ? 'FINAL' : submitted === league.members.length && submitted > 1 ? 'READY' : 'DRAFT'}</div></div><div><div className="text-[9px] font-black uppercase tracking-widest text-zinc-600">Owners Ready</div><div className="mt-1 text-sm font-black">{submitted}/{league.members.length}</div></div><div><div className="text-[9px] font-black uppercase tracking-widest text-zinc-600">Your Roster</div><div className="mt-1 text-sm font-black">{mine?.status === 'ready' ? 'LOCKED' : 'BUILD'}</div></div><div><div className="text-[9px] font-black uppercase tracking-widest text-zinc-600">Cap</div><div className="mt-1 text-sm font-black">${league.salaryCap}M</div></div></div>
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center"><button onClick={() => onSelectLeague(league, completed ? 'simulation' : 'lobby')} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-4 py-3 text-xs font-black uppercase tracking-wider hover:border-[#D4AF37]/35">{completed ? <Trophy className="h-4 w-4"/> : <Users className="h-4 w-4"/>}{completed ? 'View Results' : 'League Lobby'}</button>{!completed && <button onClick={() => onSelectLeague(league, 'draft')} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#D4AF37] px-4 py-3 text-xs font-black uppercase tracking-wider text-black">{mine?.status === 'ready' ? 'View Draft Board' : 'Build Your Team'}<ArrowRight className="h-4 w-4"/></button>}</div></div></article>;
        })}</div>}

        <div className="mt-8 grid grid-cols-4 gap-2 rounded-2xl border border-white/10 bg-[#0b0d10]/90 p-3 sm:grid-cols-7">{[[Trophy,'Leagues'],[Shield,'Draft'],[Users,'Teams'],[BarChart3,'Standings'],[CalendarDays,'Schedule'],[MessageSquare,'Activity'],[MoreHorizontal,'More']].map(([Icon,label]: any) => <div key={label} className="flex flex-col items-center gap-1.5 rounded-xl px-2 py-2 text-[9px] font-black uppercase tracking-wider text-zinc-500 first:text-[#D4AF37]"><Icon className="h-5 w-5"/><span>{label}</span></div>)}</div>
      </div>
    </div>
  );
};
