...
import {fetchSeasonOperations,resolveTrade,TradeAction,TradeOffer} from './fantasySeasonCloud';
...
export const FantasyTradeReviewPanel:React.FC<{league:League}>=({league})=>{
  ...
  const act=async(id:string,action:Extract<TradeAction,'approved'|'vetoed'>)=>{if(busy)return;setBusy(true);try{await resolveTrade(id,action);showToast(action==='approved'?'Trade approved and completed.':'Trade vetoed.');await refresh();}catch(err:any){showToast(err?.message||'Trade review failed.');}finally{setBusy(false);}};
