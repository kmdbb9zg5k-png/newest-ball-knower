      ...
      <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-[760px] flex-col px-4 sm:px-5 pt-[max(20px,env(safe-area-inset-top))] pb-[max(30px,calc(env(safe-area-inset-bottom)+24px))]">
        ...
        >
          <div className="absolute left-1/2 top-0 z-40 -translate-x-1/2">
            <div className="h-0 w-0 border-l-[14px] border-r-[14px] border-t-[24px] border-l-transparent border-r-transparent" style={{borderTopColor: preview.secondary, filter:`drop-shadow(0 0 12px ${preview.secondary}99)`}} />
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderColor:active?t.secondary:'rgba(255,255,255,.10)',
                    background:active?`linear-gradient(180deg,${t.primary}E6 0%,${t.secondary}2E 42%,rgba(5,8,10,.96) 84%)`:'linear-gradient(180deg,rgba(22,27,32,.92),rgba(4,6,8,.96))',
                    boxShadow:active?`0 0 0 1px ${t.secondary}88 inset, 0 24px 65px ${t.primary}66, 0 0 30px ${t.secondary}33`:'0 18px 45px rgba(0,0,0,.6)',
                  }}
                >
          ...
          <button onClick={() => move(-1)} aria-label="Previous team" className="absolute bottom-3 left-2 z-50 grid h-12 w-12 place-items-center rounded-full border bg-black/70 text-2xl text-white backdrop-blur active:scale-95" style={{borderColor:`${preview.secondary}88`, boxShadow:`0 0 18px ${preview.primary}44`}}>‹</button>
          <button onClick={() => move(1)} aria-label="Next team" className="absolute bottom-3 right-2 z-50 grid h-12 w-12 place-items-center rounded-full border bg-black/70 text-2xl text-white backdrop-blur active:scale-95" style={{borderColor:`${preview.secondary}88`, boxShadow:`0 0 18px ${preview.primary}44`}}>›</button>
        </div>
