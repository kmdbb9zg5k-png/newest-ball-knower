...
export const FranchiseInteractionCenter: React.FC<Props> = ({ state, roster, onRespond, onUpgrade, onReadAll }) => {
  ...
  return <section aria-label="Franchise interactions" className="mb-4 space-y-3">
    ...
    {showUpgrades ? <div className="rounded-2xl border border-violet-400/20 bg-violet-400/[.04] p-3"><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase text-violet-300"><Sparkles className="h-4 w-4" />Player upgrades</div>{upgrades.length ? <div className="space-y-2">{upgrades.slice(0, 6).map(development => { const player = playerById.get(development.playerId)!; return <div key={player.id} className="rounded-xl border border-white/10 bg-black/25 p-3"><div className="flex items-center justify-between gap-2"><div className="min-w-0"><div className="truncate text-sm font-black">{player.name}</div><div className="text-[10px] text-zinc-500">{player.position} · {player.ovr} OVR · {development.xp}/100 XP</div></div><b className="text-violet-300">{development.upgradePoints} PT</b></div><div className="mt-2 grid grid-cols-3 gap-1">{([['physical', 'Physical'], ['awareness', 'Awareness'], ['position', 'Position']] as Array<[UpgradeFocus, string]>).map(([focus, label]) => <button key={focus} type="button" onClick={() => onUpgrade(player.id, focus)} className="min-h-10 rounded-lg border border-violet-300/20 bg-violet-300/[.06] px-1 text-[8px] font-black uppercase text-violet-200">+1 {label}</button>)}</div></div>; })}</div> : <p className="text-xs leading-5 text-zinc-500">Players earn XP from weekly production, promises and breakout performances. Every 100 XP becomes an upgrade point.</p>}</div> : null}
