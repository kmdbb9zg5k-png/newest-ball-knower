...
export type OwnerTeamBaseline = {
  abbr:string; team:string; stadium:string; capacity:number; valuationB:number; valuationSource:'Forbes 2025 estimate'; stadiumSource:'public stadium capacity';
};
...
].map(([abbr,team,stadium,capacity,valuationB])=>({abbr:String(abbr),team:String(team),stadium:String(stadium),capacity:Number(capacity),valuationB:Number(valuationB),valuationSource:'Forbes 2025 estimate' as const,stadiumSource:'public stadium capacity' as const}));

const clamp=(n:number,min:number,max:number)=>Math.max(min,Math.min(max,n));
const money=(n:number)=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n);

export const OwnerBusinessMode:React.FC<{onBack:()=>void}>=({onBack})=>{
 const [abbr,setAbbr]=useState('PHI');
 const [pricing,setPricing]=useState<Pricing>({ticket:165,parking:45,food:13,merch:125});
 const [facility,setFacility]=useState(72);
 const [marketing,setMarketing]=useState(65);
 const [staff,setStaff]=useState(78);
 const team=TEAMS.find(t=>t.abbr===abbr)||TEAMS[0];
 const model=useMemo(()=>{
   ...
   const homeGames=9;
   const tickets=attendance*homeGames*pricing.ticket;
   const parking=attendance*homeGames*.42*pricing.parking;
   const concessions=attendance*homeGames*.77*pricing.food*1.9;
   const merch=attendance*homeGames*.18*pricing.merch;
   ...
   return {affordability,fanSatisfaction,attendance,localRevenue,operatingContribution,projectedValue:team.valuationB*(1+valueChange)};
 ...
 return <div className="min-h-[100dvh] px-4 py-5 text-white sm:px-8"><div className="mx-auto max-w-6xl">
   ...
   <section className="overflow-hidden rounded-[2rem] border border-[var(--bk-team-accent)]/40 bg-[#090c12]/90 p-5 shadow-2xl sm:p-7">
    <div className="grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
     <div><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-[10px] font-black tracking-[.25em] text-zinc-500">REAL-WORLD BASELINE</div><h2 className="mt-1 text-4xl font-black sm:text-5xl">{team.team}</h2><p className="mt-2 text-sm font-semibold text-zinc-400">{team.stadium} · {team.capacity.toLocaleString()} seats</p></div><select value={abbr} onChange={e=>setAbbr(e.target.value)} className="rounded-xl border border-white/10 bg-black px-3 py-3 text-sm font-black">{TEAMS.map(t=><option key={t.abbr} value={t.abbr}>{t.abbr} — {t.team}</option>)}</select></div>
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">{[
       ['FAN SATISFACTION',`${model.fanSatisfaction}/100`,Heart],['PROJECTED ATTENDANCE',model.attendance.toLocaleString(),Users],['LOCAL REVENUE',money(model.localRevenue),DollarSign],['FRANCHISE VALUE',`$${model.projectedValue.toFixed(2)}B`,TrendingUp]
      ].map(([l,v,I]:any)=><div key={l} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><I size={18} className="text-[var(--bk-team-accent)]"/><div className="mt-3 text-[9px] font-black tracking-wider text-zinc-500">{l}</div><div className="mt-1 text-lg font-black">{v}</div></div>)}</div>
     </div>
     <div className="rounded-3xl border border-white/10 bg-black/30 p-5"><div className="text-xs font-black uppercase tracking-widest text-[var(--bk-team-accent)]">Owner Objective</div><div className="mt-3 text-3xl font-black">KEEP FANS HAPPY.</div><p className="mt-3 text-sm leading-relaxed text-zinc-400">Winning matters, but so do affordability, stadium quality and trust. Raise prices too aggressively and demand falls. Invest wisely and long-term franchise value grows.</p><div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-zinc-400"><b className="text-white">Data transparency:</b> stadium capacity is a public real-world baseline. Franchise value uses a 2025 Forbes estimate. Revenue, attendance response and future value are Ball Knower simulation outputs.</div></div>
    </div>
   </section>

   <div className="mt-5 grid gap-5 lg:grid-cols-2">
    <section className="rounded-[2rem] border border-white/10 bg-[#10151d]/90 p-5 sm:p-6"><div className="mb-4 flex items-center gap-2"><Ticket className="text-[var(--bk-team-accent)]"/><h3 className="text-2xl font-black">STADIUM ECONOMICS</h3></div><div className="space-y-3">{slider('Average ticket',pricing.ticket,60,400,5,v=>setPricing(p=>({...p,ticket:v})),<Ticket size={15}/>,'$')}{slider('Parking',pricing.parking,10,120,5,v=>setPricing(p=>({...p,parking:v})),<MapPin size={15}/>,'$')}{slider('Average food basket',pricing.food,5,35,1,v=>setPricing(p=>({...p,food:v})),<UtensilsCrossed size={15}/>,'$')}{slider('Average merch item',pricing.merch,50,250,5,v=>setPricing(p=>({...p,merch:v})),<Shirt size={15}/>,'$')}</div></section>
    <section className="rounded-[2rem] border border-white/10 bg-[#10151d]/90 p-5 sm:p-6"><div className="mb-4 flex items-center gap-2"><Building2 className="text-[var(--bk-team-accent)]"/><h3 className="text-2xl font-black">ORGANIZATION</h3></div><div className="space-y-3">{slider('Facilities',facility,25,100,1,setFacility,<Building2 size={15}/>,'%')}{slider('Marketing',marketing,25,100,1,setMarketing,<TrendingUp size={15}/>,'%')}{slider('Staff investment',staff,25,100,1,setStaff,<Users size={15}/>,'%')}</div><div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-4"><div className="flex items-center justify-between text-xs font-black"><span>OPERATING CONTRIBUTION</span><span className={model.operatingContribution>=0?'text-emerald-300':'text-red-300'}>{money(model.operatingContribution)}</span></div><div className="mt-2 text-[11px] leading-relaxed text-zinc-500">Modeled local-business contribution only. National media distributions, debt service, taxes and confidential club expenses are intentionally excluded until reliable data is available.</div></div></section>
   </div>

   <section className="mt-5 rounded-[2rem] border border-white/10 bg-[#10151d]/90 p-5 sm:p-6"><div className="flex items-center gap-2"><MapPin className="text-[var(--bk-team-accent)]"/><h3 className="text-2xl font-black">RELOCATION & MARKET STUDY</h3></div><p className="mt-2 max-w-3xl text-sm text-zinc-400">Relocation will be a multi-step owner decision: market study → stadium plan → fan backlash/approval → league vote → rebrand. Market population, income, corporate base and competing sports will feed the approval and revenue model.</p><div className="mt-4 grid gap-3 sm:grid-cols-3">{['San Antonio','St. Louis','Portland'].map((city,i)=><div key={city} className="rounded-2xl border border-white/10 bg-black/25 p-4"><div className="text-lg font-black">{city}</div><div className="mt-2 text-[10px] font-black tracking-wider text-zinc-500">MARKET STUDY</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-[var(--bk-team-accent)]" style={{width:`${[82,74,69][i]}%`}}/></div><div className="mt-2 text-xs text-zinc-400">Opportunity score {[82,74,69][i]}/100 · preliminary simulated score</div></div>)}</div></section>
  </div></div>;
};
