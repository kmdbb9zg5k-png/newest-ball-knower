...
import {Appearance,BodyBuild,FACE_COUNT,saveAppearance,UniformVariant,uniformFor,playerAttributes} from './appearance';
...
const sameLook=(a:Appearance,b:Appearance)=>a.face===b.face&&a.build===b.build&&a.number===b.number&&a.sleeves===b.sleeves;
    ...
    <section className="bk-solo-profile" role="dialog" aria-modal="true" aria-labelledby={`${id}-name`} style={{'--solo-kit':kit.jersey,'--solo-trim':kit.trim} as React.CSSProperties}>
      ...
      <div className="bk-solo-profile-scroll">
        ...
        <div id={`${id}-panel`} role="tabpanel" aria-labelledby={`${id}-${tab}`} className="bk-solo-profile-panel">
          ...
          {tab==='appearance'&&<><h2>Edit player appearance</h2><p className="bk-solo-muted">Saved on this device. This changes the character preview, not ratings, contracts or career progress.</p><UniformSwitch value={variant} onChange={setVariant}/><div className="bk-solo-editor-layout"><div className="bk-solo-editor-model"><SoloCharacter player={player} look={draft} variant={variant}/></div><div className="bk-solo-editor-controls">
            ...
            <fieldset><legend>Body build</legend><div className="bk-solo-choice-stack">{(['lean','athletic','power'] as BodyBuild[]).map(build=><button key={build} type="button" aria-pressed={draft.build===build} onClick={()=>setDraft(value=>({...value,build}))}>{build}</button>)}</div></fieldset>
            ...
            <fieldset><legend>Arm gear</legend><div className="bk-solo-choice-stack">{(['right','both'] as const).map(sleeves=><button key={sleeves} type="button" aria-pressed={draft.sleeves===sleeves} onClick={()=>setDraft(value=>({...value,sleeves}))}>{sleeves==='right'?'Right sleeve':'Both sleeves'}</button>)}</div></fieldset>
          </div></div></>}
        </div>
