-- Re-enter an unfinished public league instead of silently creating a duplicate
-- after its draft order has been locked.
...
begin
  ...
  left join public.ball_knower_live_drafts d on d.league_id = l.id
  where m.auth_user_id = v_auth
    and (l.settings ->> 'leagueType') = 'public_free'
    and (
      l.status = 'drafting'
      or d.status = 'active'
      or (l.status = 'completed' and d.league_id is null)
    )
  order by
    case when d.status = 'active' then 0 when l.status = 'drafting' then 1 else 2 end,
    greatest(l.updated_at, coalesce(d.updated_at, l.updated_at)) desc
  limit 1;
  ...
  if v_league_id is null then
    ...
    loop
      ...
      exit when not exists (select 1 from public.ball_knower_leagues where code = v_code);
    ...
    insert into public.ball_knower_leagues(
      ...
      v_league_id, v_code, 'Open Ball Knower ' || right(v_code, 3), v_max, 301.2,
      v_auth, v_name, 'drafting',
      jsonb_build_object(
        'seasonGames', 17,
  ...
  insert into public.ball_knower_league_members(
    ...
    v_league_id, v_auth, v_auth::text, v_name, left(p_user_avatar, 1000),
    not exists (select 1 from public.ball_knower_league_members where league_id = v_league_id),
    false, 'building'
  )
