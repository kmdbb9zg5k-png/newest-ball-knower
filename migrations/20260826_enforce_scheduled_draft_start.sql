-- Keep the scheduled draft time authoritative at the database boundary. The
-- client also blocks early starts, but this prevents older clients or direct
-- RPC calls from bypassing the commissioner-selected schedule.
...
begin
  ...
  if v_scheduled_at is not null and now() < v_scheduled_at then
    raise exception 'The fantasy draft is scheduled for %', v_scheduled_at;
  end if;
