-- Keep live fantasy drafts moving if the commissioner closes or backgrounds the app.
-- Human turns remain restricted to the manager on the clock. CPU turns may be
-- advanced by any authenticated human member of the league; the row lock still
-- makes the pick operation atomic when multiple clients try at once.
...
begin
  ...
  if v_team_count < 1 or v_draft.pick_index >= v_total_picks then
    raise exception 'Fantasy draft is already complete';
  end if;
  ...
  if coalesce(v_member.is_ai, false) then
    if not exists (
      ...
      raise exception 'Only league members can complete CPU picks';
    end if;
  ...
  where pick ->> 'memberId' = v_member_id
    and pick ->> 'group' = v_canonical_group;
  if v_group_count >= v_group_limit then
    raise exception '% reached the % roster limit', v_member.user_name, v_canonical_group;
  end if;
