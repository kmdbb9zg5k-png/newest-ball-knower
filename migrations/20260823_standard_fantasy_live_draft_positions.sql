...
declare
  ...
  v_pick jsonb; v_next_index integer;
begin
  ...
  v_group_limit := case p_group
    when 'QB' then 2 when 'RB' then 5 when 'WR' then 6 when 'TE' then 3
  ...
  select count(*) into v_group_count from jsonb_array_elements(v_draft.picks) pick
    where pick ->> 'memberId' = v_member_id and pick ->> 'group' = p_group;
  if v_group_count >= v_group_limit then raise exception '% already filled every % roster slot', v_member.user_name, p_group; end if;

  v_pick := jsonb_build_object('overall',v_draft.pick_index+1,'round',v_round_index+1,'memberId',v_member_id,'playerId',p_player_id,'group',p_group,'pickedAt',clock_timestamp());
