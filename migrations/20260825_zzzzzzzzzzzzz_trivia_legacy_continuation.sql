-- Keep the legacy one-argument Trivia RPC compatible without letting it take over
-- a verified token-bound session. Existing legacy state may continue under its own
-- token; any non-legacy registered state remains fail-closed.
...
begin
  ...
  if v_state.user_id is not null then
    -- Never supersede a token registered by a current client. A legacy wrapper may
    -- only continue state that it originally created itself.
    if v_state.session_token not like 'legacy-%' then
      raise exception 'A verified trivia session is already registered';
    end if;
    v_token:=v_state.session_token;
  else
