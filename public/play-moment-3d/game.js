...
export function receiverSlotForKey(key){const slot={x:0,y:1,z:2,'1':0,'2':1,'3':2}[normalizeControlKey(key)];return Number.isInteger(slot)?slot:-1}
...
export function passOutcomeChances(separation,pressure,kind='bullet',receiverGap=0,defenderLeverage=0){
 ...
 return{
  interception:clamp(.012+Math.max(0,1.45-separation)*.13+pressure*.085+defenderLeverage*.12+profile.pickRisk,.01,.38),
  inaccurate:clamp(Math.max(0,receiverGap-.55)*.31+pressure*.075+profile.error*.04,0,.72),
  breakup:clamp(catchBreakupChance(separation)*.72+pressure*.045,0,.78),
 };
}
...
export function start(){
 ...
 let mode='run',selected=0,assist=false,phase='pre',paused=false,ended=false,flight=null,carrier=null,jukeUntil=0,jukeReady=0,simTime=0;
 ...
 function blockers(dt,isRun){
  ...
  for(let i=0;i<4;i++){const d=actors[11+i],p=actors[i],release=2.15+i*.32;if(elapsed<release){chase(d,p.x,p.z+.9,4,dt);d.engaged=true;d.heading=Math.PI}else{d.engaged=false;chase(d,carrier.x,carrier.z,5.25+i*.08,dt)}}
 }
 ...
 function tick(dt){if(phase==='dead'){if(!ended){recoveryLeft-=dt;if(recoveryLeft<=0)setup()}return}if(phase==='pre')return;elapsed+=dt;simTime+=dt;drive.clock=Math.max(0,drive.clock-dt);updateHud();
  ...
  if(phase==='run'){
   ...
   blockers(dt,mode==='run');for(const p of actors.filter(p=>p.team===1&&p.role!=='DL'&&!p.engaged)){const separation=Math.hypot(p.x-carrier.x,p.z-carrier.z);chase(p,carrier.x,carrier.z+1,defenderPursuitSpeed(separation,mode==='pass'),dt)}
   ...
   if(simTime>jukeUntil){const radius=tackleRadius(elapsed,mode==='pass'),d=radius&&actors.find(p=>p.team===1&&!p.engaged&&Math.hypot(p.x-carrier.x,p.z-carrier.z)<radius);if(d){endPlay('TACKLED',carrier.z-10);return}}
 ...
 const juke=()=>{if(phase!=='run'||simTime<jukeReady)return;const keyboardX=(keys.has('ArrowRight')||keys.has('d')?1:0)-(keys.has('ArrowLeft')||keys.has('a')?1:0),[rightX,rightZ]=cameraWorldVector(1,0,camEye,camTarget),nearest=actors.filter(p=>p.team===1&&!p.engaged).reduce((best,p)=>!best||Math.hypot(p.x-carrier.x,p.z-carrier.z)<Math.hypot(best.x-carrier.x,best.z-carrier.z)?p:best,null),defenderSide=nearest?(nearest.x-carrier.x)*rightX+(nearest.z-carrier.z)*rightZ:0,side=Math.abs(keyboardX||input.x)>.15?Math.sign(keyboardX||input.x):(defenderSide>0?-1:1),[dx,dz]=cameraWorldVector(side,0,camEye,camTarget);jukeUntil=simTime+.75;jukeReady=simTime+1.25;const targetHeading=Math.atan2(dx,dz),headingDiff=Math.atan2(Math.sin(targetHeading-carrier.heading),Math.cos(targetHeading-carrier.heading));carrier.heading+=headingDiff*.72;carrier.x=clamp(carrier.x+dx*2.05,-25.8,25.8);carrier.z=clamp(carrier.z+dz*2.05,10,110);message('JUKE '+(side>0?'RIGHT':'LEFT'),.65);navigator.vibrate?.(20)};$('juke').onpointerdown=e=>{juke();$('juke').setPointerCapture(e.pointerId);e.preventDefault()};$('juke').onclick=juke;
 window.addEventListener('keydown',e=>{if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code))e.preventDefault();const key=normalizeControlKey(e.key),receiverSlot=receiverSlotForKey(key);keys.add(key);if(e.code==='Space'&&!e.repeat)snap();if(receiverSlot>=0&&!e.repeat)throwTo(receiverIndices[receiverSlot],throwKindForModifiers(e.shiftKey,e.altKey));if(key==='Escape'&&!e.repeat)pause();if(key==='j')juke()});window.addEventListener('keyup',e=>keys.delete(normalizeControlKey(e.key)));
 ...
 if(new URLSearchParams(location.search).has('qa')){window.bk3dTest={manualFrames(){qaStepping=true;accumulator=0;cancelAnimationFrame(raf)},step(seconds){const n=Math.ceil(clamp(seconds,0,10)*60);for(let i=0;i<n;i++){if(!paused)simulate(1/60);camera(1/60)}scene(.016,performance.now())}};window.bk3dDiagnostics=()=>{const qb=actors[5],nearest=Math.min(...actors.slice(11,15).filter(p=>!p.engaged).map(p=>Math.hypot(p.x-qb.x,p.z-qb.z)),8);return({phase,mode,selected,assist,elapsed,simTime,paused,ended,throwKind:flight?.kind||null,pocketPressure:phase==='pass'?pocketPressure(nearest,elapsed):0,frames:frameId,drawCalls:r.drawCalls,glError:r.gl.getError(),players:actors.map(p=>({role:p.role,team:p.team,x:p.x,z:p.z,distance:p.distance,heading:p.heading,hasBall:p.hasBall,engaged:p.engaged,pose:p.motion?{speed:p.motion.speed,run:p.motion.run,ready:p.motion.ready,block:p.motion.block,turn:p.motion.turn,gait:p.motion.gait,fall:p.motion.fall}:null,head:r.project([p.x,2.1,p.z]),foot:r.project([p.x,0,p.z])})),drive:{...drive},field:{scrimmage:snapZ,lineToGain:snapGainZ},stamina,worldObjects:stadium.parts})};}
}
