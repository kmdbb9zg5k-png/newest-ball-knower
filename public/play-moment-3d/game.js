...
export function start(){
 ...
 function tick(dt){if(phase==='dead'){if(!ended){recoveryLeft-=dt;if(recoveryLeft<=0)setup()}return}if(phase==='pre')return;elapsed+=dt;simTime+=dt;drive.clock=Math.max(0,drive.clock-dt);updateHud();
  ...
  if(phase==='pass'||phase==='flight'){coverage(dt);blockers(dt,false);chase(actors[5],0,snapZ-5-Math.min(elapsed*.4,1.2),.55,dt);actors[5].heading=0;
   ...
   if(flight){flight.t+=dt/flight.duration;actors[5].throwT=clamp(flight.t,.001,1);if(flight.t>=1){const p=actors[flight.target],distance=Math.min(...actors.filter(a=>a.team===1).map(a=>Math.hypot(a.x-p.x,a.z-p.z))),breakupChance=catchBreakupChance(distance);flight=null;actors[5].throwT=0;if(rand()<breakupChance){const miss=distance>=2?'DROPPED PASS':distance<.7?'TIGHT WINDOW · PASS BROKEN UP':'PASS BROKEN UP';endPlay(miss,drive.ball,true);return}p.hasBall=true;p.catchT=.35;carrier=p;phase='run';elapsed=0;updateControls();message(distance<2?'CONTESTED CATCH · TAKE CONTROL':'COMPLETE · TAKE CONTROL',1)}}
   return;
  }
  if(phase==='run'){
 ...
 window.addEventListener('keydown',e=>{if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code))e.preventDefault();const key=normalizeControlKey(e.key),receiverSlot=receiverSlotForKey(key);keys.add(key);if(e.code==='Space'&&!e.repeat)snap();if(receiverSlot>=0)throwTo(receiverIndices[receiverSlot]);if(key==='Escape'&&!e.repeat)pause();if(key==='j')juke()});window.addEventListener('keyup',e=>keys.delete(normalizeControlKey(e.key)));
 ...
 if(new URLSearchParams(location.search).has('qa')){window.bk3dTest={manualFrames(){qaStepping=true;accumulator=0;cancelAnimationFrame(raf)},step(seconds){const n=Math.ceil(clamp(seconds,0,10)*60);for(let i=0;i<n;i++){if(!paused)simulate(1/60);camera(1/60)}scene(.016,performance.now())}};window.bk3dDiagnostics=()=>({phase,mode,selected,assist,elapsed,simTime,paused,ended,frames:frameId,drawCalls:r.drawCalls,glError:r.gl.getError(),players:actors.map(p=>({role:p.role,team:p.team,x:p.x,z:p.z,distance:p.distance,heading:p.heading,hasBall:p.hasBall,pose:p.motion?{speed:p.motion.speed,run:p.motion.run,ready:p.motion.ready,block:p.motion.block,turn:p.motion.turn,gait:p.motion.gait,fall:p.motion.fall}:null,head:r.project([p.x,2.1,p.z]),foot:r.project([p.x,0,p.z])})),drive:{...drive},field:{scrimmage:snapZ,lineToGain:snapGainZ},stamina,worldObjects:stadium.parts});}
}
