...
export function start(){
 ...
 function blockers(dt,isRun){
  for(let i=0;i<5;i++){const p=actors[i],lane=isRun?RUNS[selected].path[1][0]*.28:0,desiredZ=snapZ+(isRun?Math.min(elapsed*1.7,4):-.4-Math.min(elapsed*.55,1.7));p.engaged=true;chase(p,p.startX+lane,desiredZ,2.8,dt);p.heading=0}
  for(let i=0;i<4;i++){const d=actors[11+i],p=actors[i],release=isRun?2.3+(i%2)*.4:2.15+i*.32;if(elapsed<release){chase(d,p.x,p.z+.9,4,dt);d.engaged=true;d.heading=Math.PI}else{d.engaged=false;chase(d,carrier.x,carrier.z,5.25+i*.08,dt)}}
 }
 ...
 function tick(dt){if(phase==='dead'){if(!ended){recoveryLeft-=dt;if(recoveryLeft<=0)setup()}return}if(phase==='pre')return;elapsed+=dt;simTime+=dt;drive.clock=Math.max(0,drive.clock-dt);updateHud();
  ...
  if(phase==='run'){
   ...
   blockers(dt,true);for(const p of actors.filter(p=>p.team===1&&p.role!=='DL')){const separation=Math.hypot(p.x-carrier.x,p.z-carrier.z);chase(p,carrier.x,carrier.z+1,defenderPursuitSpeed(separation,mode==='pass'),dt)}
 ...
 const juke=()=>{if(phase!=='run'||simTime<jukeReady)return;const keyboardX=(keys.has('ArrowRight')||keys.has('d')?1:0)-(keys.has('ArrowLeft')||keys.has('a')?1:0),side=(keyboardX||input.x)<0?-1:1,[dx,dz]=cameraWorldVector(side,0,camEye,camTarget);jukeUntil=simTime+.52;jukeReady=simTime+1.5;const targetHeading=Math.atan2(dx,dz),headingDiff=Math.atan2(Math.sin(targetHeading-carrier.heading),Math.cos(targetHeading-carrier.heading));carrier.heading+=headingDiff*.45;carrier.x=clamp(carrier.x+dx*1.15,-25.8,25.8);carrier.z=clamp(carrier.z+dz*1.15,10,110);message('JUKE '+(side>0?'RIGHT':'LEFT'),.45)};$('juke').onclick=juke;
