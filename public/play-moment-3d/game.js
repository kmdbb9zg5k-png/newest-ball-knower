...
export function start(){
 ...
 function throwTo(index){if(phase!=='pass'||paused)return;const target=actors[index];carrier.hasBall=false;actors[5].throwT=.001;flight={from:[carrier.x,1.55,carrier.z],to:[target.x,1.6,target.z+2],target:index,t:0,duration:.60+Math.abs(target.z-carrier.z)*.008};phase='flight';updateControls()}
 ...
 function tick(dt){if(phase==='dead'){if(!ended){recoveryLeft-=dt;if(recoveryLeft<=0)setup()}return}if(phase==='pre')return;elapsed+=dt;drive.clock=Math.max(0,drive.clock-dt);updateHud();
  ...
  if(phase==='run'){
   ...
   if(!assist){const fx=camTarget[0]-camEye[0],fz=camTarget[2]-camEye[2],l=Math.hypot(fx,fz)||1;const worldX=(-fz*x+fx*z)/l,worldZ=(fx*x+fz*z)/l;x=worldX;z=worldZ}
 ...
 function scene(dt,now){r.begin();stadium.draw();
  ...
  if(flight){const t=clamp(flight.t,0,1),p=flight.from.map((v,i)=>v+(flight.to[i]-v)*t);p[1]+=Math.sin(Math.PI*t)*3.3;r.add('sphere',pose(...p,.12,.12,.23),hex('#7d4226'));r.add('plane',pose(p[0],.06,p[2],.45,1,.35),[0,0,0,.65],'shadow',true)}
  r.draw();
 ...
 const juke=()=>{const now=performance.now();if(phase!=='run'||now<jukeReady)return;jukeUntil=now+520;jukeReady=now+1500;carrier.x=clamp(carrier.x+(input.x>=0?1.15:-1.15),-25.8,25.8);message('JUKE',.45)};$('juke').onclick=juke;
 window.addEventListener('keydown',e=>{if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code))e.preventDefault();keys.add(e.key);if(e.code==='Space'&&!e.repeat)snap();if(['1','2','3'].includes(e.key))throwTo(receiverIndices[Number(e.key)-1]);if(e.key==='Escape'&&!e.repeat)pause();if(e.key==='j')juke()});window.addEventListener('keyup',e=>keys.delete(e.key));
