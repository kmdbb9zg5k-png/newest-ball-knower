...
const PlayerRow = ({ player, action }: { player: Player; action: React.ReactNode }) => <div className="flex min-w-0 items-center gap-3 rounded-2xl border border-white/10 bg-[#111] p-3"><PlayerThumb player={player}/><div className="min-w-0 flex-1"><div className="truncate font-black">{player.name}</div><div className="text-[10px] font-bold text-zinc-500">{player.position} • {getDraftPositionGroup(player)} • {player.ovr} OVR • {money(player.salary)}</div></div>{action}</div>;

const SelectPlayer = ({ player, selected, onClick }: { player: Player; selected: boolean; onClick: () => void }) => <button type="button" aria-pressed={selected} onClick={onClick} className={`flex w-full min-w-0 items-center gap-3 rounded-2xl border p-3 text-left ${selected ? 'border-[var(--bk-team-accent)] bg-[var(--bk-team-accent)]/10' : 'border-white/10 bg-black/20'}`}><PlayerThumb player={player}/><div className="min-w-0 flex-1"><div className="truncate font-black">{player.name}</div><div className="text-[10px] font-bold text-zinc-500">{player.position} • {player.ovr} OVR • {money(player.salary)}</div></div></button>;
