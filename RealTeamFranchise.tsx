  ...
  if (teamAbbr) {
    ...
    const releasePlayer = (player: Player) => {
      if (roster.length <= TOTAL_ROSTER_SIZE) {
        setMessage(`Release blocked. A legal franchise roster needs ${TOTAL_ROSTER_SIZE} players. Use trades to replace a player instead.`);
        return;
      }
    ...
    const submitOffer = () => {
      ...
      const proposedRoster = buildManagedRosters(nextMoves, rookies)[teamAbbr] ?? [];
      if (proposedRoster.length < TOTAL_ROSTER_SIZE) {
        setMessage(`Trade blocked. Your roster would fall below the ${TOTAL_ROSTER_SIZE}-player minimum.`);
        return;
      }
    ...
    return <div className="min-h-[100dvh] px-4 pb-24 pt-4 text-white sm:px-8" style={themeStyle}><div className="mx-auto max-w-6xl">
      ...
      {commandTab === 'roster' ? <section className="mt-3 rounded-[2rem] border border-white/10 bg-[#10151d]/90 p-4 backdrop-blur sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-3xl font-black">YOUR ROSTER</h2><p className="mt-1 text-sm text-zinc-500">{roster.length}/{TOTAL_ROSTER_SIZE} players · ${capUsed.toFixed(1)}M/${DEFAULT_SALARY_CAP.toFixed(1)}M cap</p></div><div className={`rounded-full px-3 py-2 text-[10px] font-black ${rosterLegal ? 'bg-green-400/15 text-green-300' : 'bg-amber-300/15 text-amber-200'}`}>{rosterLegal ? 'ROSTER LEGAL' : 'CUTDOWN REQUIRED'}</div></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{roster.slice().sort((a, b) => b.ovr - a.ovr).map(player => <div key={player.id} className="flex items-center gap-3 rounded-xl bg-black/25 p-3"><div className="min-w-0 flex-1"><div className="truncate text-sm font-black">{player.name}</div><div className="text-[10px] text-zinc-500">{player.position} · ${player.salary}M</div></div><div className="text-lg font-black text-[var(--bk-team-accent)]">{player.ovr}</div><button type="button" onClick={() => releasePlayer(player)} className="min-h-11 shrink-0 rounded-lg border border-red-400/25 px-3 text-[9px] font-black text-red-300">RELEASE</button></div>)}</div><div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-4 text-xs text-zinc-400"><b className="text-[var(--bk-team-accent)]">DRAFT CAPITAL:</b> {ownedDraftRounds.length ? ownedDraftRounds.map(round => `${ordinal(round)} round`).join(' · ') : 'No picks remaining'}</div></section> : null}
