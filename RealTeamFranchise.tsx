...
export const RealTeamFranchise: React.FC<Props> = ({ onBack }) => {
  ...
  if (teamAbbr) {
    ...
    return <div className="min-h-[100dvh] px-4 pb-24 pt-4 text-white sm:px-8"><div className="mx-auto max-w-6xl">
      ...
      {commandTab === 'trade' && <section className="mt-3 rounded-[2rem] border border-white/10 bg-[#10151d] p-4 sm:p-6"><div className="text-[10px] font-black uppercase tracking-[.22em] text-emerald-300">All 32 teams</div><h2 className="mt-2 text-3xl font-black">TRADE CENTER</h2><p className="mt-2 text-sm text-zinc-400">Choose a target, then build the offer yourself with your players and draft picks.</p><input value={tradeSearch} onChange={event => setTradeSearch(event.target.value)} placeholder="Search player, team or position" className="mt-4 min-h-12 w-full rounded-xl border border-white/10 bg-black/30 px-4 text-sm outline-none"/><div className="mt-3 divide-y divide-white/5">{tradeTargets.map(player => <div key={player.id} className="flex items-center gap-3 py-3"><div className="min-w-0 flex-1"><div className="truncate text-sm font-black">{player.name}</div><div className="text-[10px] font-bold text-zinc-500">{player.team} · {player.position} · {player.ovr} OVR · ${player.salary}M</div></div><button onClick={() => { setTradeTarget(player); setOutgoingIds([]); setOutgoingPicks([]); setMessage(''); }} className="min-h-11 shrink-0 rounded-xl border border-emerald-300/30 px-3 text-[10px] font-black text-emerald-300">BUILD OFFER</button></div>)}</div></section>}
