...
const WeekCard = ({ week }: { week: FantasyPlayerWeek }) => {
  ...
  return <section className="rounded-2xl border border-white/10 bg-black/25 p-3"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-[9px] font-black uppercase text-[#D4AF37]"><CalendarDays className="h-3.5 w-3.5" />Week {week.week}</div><div className="mt-1 text-[10px] text-zinc-500">{new Date(week.kickoffAt).toLocaleString([], { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })} · {week.status}</div></div><div className="text-right"><div className="text-lg font-black">{week.isFinal ? points(week, 'actual').toFixed(1) : points(week, 'projected').toFixed(1)}</div><div className="text-[8px] uppercase text-zinc-600">{week.isFinal ? `Actual · ${points(week, 'projected').toFixed(1)} projected` : 'Projected'}</div></div></div>{stats.length > 0 && <div className="mt-3 grid grid-cols-2 gap-1 sm:grid-cols-4">{stats.map(([key, value]) => <div key={key} className="rounded-lg bg-white/[.04] p-2"><div className="text-[7px] font-black text-zinc-600">{statLabel(key)}</div><div className="mt-0.5 text-xs font-black">{String(value)}</div></div>)}</div>}<div className="mt-3 flex items-center gap-2 text-[8px] font-black uppercase text-zinc-600"><Activity className="h-3 w-3" />Verified scoring row <Shield className="ml-auto h-3 w-3" /></div></section>;
};
