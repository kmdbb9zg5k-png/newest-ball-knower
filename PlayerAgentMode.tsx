...
const restore=():AgencyState=>{try{const raw=localStorage.getItem(SAVE_KEY)||localStorage.getItem('ballknower_player_agent_v1');if(!raw)return fallbackAgency();const v=JSON.parse(raw);return {...fallbackAgency(),...v,clients:Array.isArray(v?.clients)?v.clients.map((c:any)=>({playerId:String(c.playerId),trust:Number(c.trust)||72,careerEarnedM:Number.isFinite(c.careerEarnedM)?Number(c.careerEarnedM):null,careerEarningsSource:c.careerEarningsSource,futureDeal:c.futureDeal,signedAt:c.signedAt||new Date().toISOString()})):[],recruitCooldowns:v?.recruitCooldowns&&typeof v.recruitCooldowns==='object'?v.recruitCooldowns:{}};}catch{return fallbackAgency()}};
