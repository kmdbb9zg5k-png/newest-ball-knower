...
export const Navbar:React.FC<NavbarProps>=({currentTab,setCurrentTab,onOpenAuth,onOpenCreateLeague,onOpenJoinLeague,onOpenIntro,onOpenDatabaseModal})=>{
  ...
  const tabClass=(tab:AppTab)=>`relative h-full shrink-0 flex items-center gap-1.5 whitespace-nowrap border-b-2 px-0.5 text-[10px] sm:text-[11px] font-black uppercase tracking-[.1em] transition-colors ${currentTab===tab?'border-[var(--bk-team-accent)] text-[var(--bk-team-accent)]':'border-transparent text-zinc-500 hover:text-white'}`;

  return <header className="sticky top-0 z-40 w-full border-b border-[var(--bk-team-accent)]/15 bg-[#090c12]/90 backdrop-blur-xl">
    <div className="mx-auto flex h-[68px] max-w-7xl items-center justify-between gap-2 px-3 pt-[env(safe-area-inset-top)] sm:px-6">
      <button onClick={()=>setCurrentTab('home')} className="shrink-0 text-left"><h1 className="font-display text-[24px] font-black leading-[.85] tracking-tighter text-white">BALL <span className="text-[var(--bk-team-accent)]">KNOWER</span></h1></button>
      <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
        {activeLeague&&<div className="relative min-w-0"><button onClick={()=>setIsLeagueMenuOpen(v=>!v)} className="flex max-w-[145px] items-center gap-1.5 rounded-xl border border-white/10 bg-[#111] px-2.5 py-2 text-[10px] font-black uppercase text-zinc-300 sm:max-w-[230px]"><Trophy className="h-3.5 w-3.5 shrink-0 text-[var(--bk-team-accent)]"/><span className="truncate">{activeLeague.name}</span><ChevronDown className="h-3 w-3 shrink-0 text-zinc-600"/></button>{isLeagueMenuOpen&&<div className="absolute right-0 z-50 mt-2 w-72 max-w-[88vw] rounded-xl border border-white/10 bg-[#111] p-2 shadow-2xl"><div className="max-h-56 space-y-1 overflow-y-auto">{leagues.map(l=><button key={l.id} onClick={()=>{setActiveLeagueId(l.id);setCurrentTab(l.status==='completed'?'simulation':'lobby');setIsLeagueMenuOpen(false)}} className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-xs ${l.id===activeLeague.id?'bg-[var(--bk-team-accent)]/10 text-[var(--bk-team-accent)]':'text-zinc-300 hover:bg-white/5'}`}><span className="truncate font-black uppercase">{l.name}</span><span className="ml-3 text-[9px] text-zinc-600">{l.code}</span></button>)}</div><div className="mt-2 flex gap-1 border-t border-white/10 pt-2"><button onClick={()=>{setIsLeagueMenuOpen(false);onOpenCreateLeague()}} className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-white/5 py-2 text-[9px] font-black uppercase"><Plus className="h-3 w-3"/>Create</button><button onClick={()=>{setIsLeagueMenuOpen(false);onOpenJoinLeague()}} className="flex flex-1 items-center justify-center gap-1 rounded-lg border border-white/5 py-2 text-[9px] font-black uppercase"><Users className="h-3 w-3"/>Join</button></div></div>}</div>}
