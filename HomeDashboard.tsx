...
import {ArrowRight,Brain,Plus,RefreshCcw,Target,Trophy,Users} from 'lucide-react';
...
export const HomeDashboard:React.FC<HomeDashboardProps>=({onOpenCreateLeague,onOpenJoinLeague,onSelectLeague,onNavigate,onOpenCheatSheet})=>{
  ...
  const openPrimaryLeague=()=>primaryLeague?onSelectLeague(primaryLeague,primaryLeague.status==='completed'?'simulation':'lobby'):onNavigate('fantasy');
  ...
  return <div className="mx-auto max-w-5xl px-3 pb-8 pt-4 sm:px-6 sm:pt-6">
    <section className="grid gap-3 lg:grid-cols-[1.2fr_.8fr]">
      <div className="rounded-2xl border border-[var(--bk-team-accent)]/25 bg-[radial-gradient(circle_at_90%_0%,rgb(var(--bk-team-primary-rgb)/.24),transparent_38%),#0b0e12] p-4 sm:p-5">
        ...
        {ratingLoading?<div className="mt-4 text-sm font-bold text-zinc-500">Verifying rating…</div>:ratingError?<div className="mt-4"><div className="text-sm font-black text-red-300">Rating unavailable</div><p className="mt-1 text-xs text-zinc-600">{ratingError}</p><button onClick={()=>void loadProfile()} className="mt-3 inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/10 px-3 text-[10px] font-black uppercase"><RefreshCcw className="h-3.5 w-3.5"/>Retry</button></div>:<><div className="mt-2 flex items-end justify-between gap-4"><div><div className="text-6xl font-black leading-none text-white sm:text-7xl">{rating}</div><div className="mt-2 text-xs font-black uppercase tracking-[.16em] text-[var(--bk-team-accent)]">{tierFor(rating??0)}</div></div><div className="text-right text-[10px] font-bold uppercase leading-5 text-zinc-600">Fantasy · Picks · Trivia<br/>all feed one profile</div></div><button onClick={()=>onNavigate('locker')} className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-4 text-[10px] font-black uppercase tracking-wider text-zinc-300">View profile <ArrowRight className="h-3.5 w-3.5"/></button></>}
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#0c0f13]/95 p-4 sm:p-5">
        ...
        <div className="mt-3 grid gap-2">
          <Quick icon={<Trophy className="h-4 w-4"/>} label="Fantasy" sub={primaryLeague?`Open ${primaryLeague.name}`:'Join or create a league'} onClick={openPrimaryLeague}/>
    ...
    {primaryLeague&&<section className="mt-3 rounded-2xl border border-white/10 bg-[#0b0e12]/95 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[9px] font-black uppercase tracking-[.2em] text-zinc-600">Your Fantasy</div><div className="mt-1 truncate text-xl font-black uppercase">{primaryLeague.name}</div><div className="mt-1 text-[10px] font-bold uppercase text-zinc-600">{primaryLeague.members.length}/{primaryLeague.maxMembers} managers{myPick?` · Your pick #${myPick}`:''}</div></div><span className="shrink-0 rounded-lg border border-[var(--bk-team-accent)]/25 bg-[var(--bk-team-accent)]/10 px-2.5 py-1 text-[9px] font-black uppercase text-[var(--bk-team-accent)]">{primaryLeague.status}</span></div><button onClick={openPrimaryLeague} className="mt-3 flex min-h-11 w-full items-center justify-between rounded-xl bg-white px-4 text-[10px] font-black uppercase tracking-wider text-black"><span>Open League</span><ArrowRight className="h-4 w-4"/></button></section>}

    <section className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><Action label="Create League" icon={<Plus className="h-4 w-4"/>} onClick={onOpenCreateLeague}/><Action label="Join League" icon={<Users className="h-4 w-4"/>} onClick={onOpenJoinLeague}/><Action label="Cheat Sheet" icon={<Trophy className="h-4 w-4"/>} onClick={onOpenCheatSheet}/><Action label="Solo / Labs" icon={<ArrowRight className="h-4 w-4"/>} onClick={()=>onNavigate('solo')}/></section>
