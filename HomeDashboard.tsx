...
export const HomeDashboard:React.FC<HomeDashboardProps>=({onOpenCreateLeague,onOpenJoinLeague,onSelectLeague,onNavigate,onOpenCheatSheet,teamTheme})=>{
  ...
  return <div className="bk-home-dashboard bk-home-broadcast mx-auto max-w-5xl pb-5 pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] pt-4 sm:pb-8 sm:pl-[max(1.5rem,env(safe-area-inset-left))] sm:pr-[max(1.5rem,env(safe-area-inset-right))] sm:pt-6">
    <HomeStadiumHero onMyLeagues={()=>onNavigate('fantasy')}/>
    <section aria-label="Primary destinations" className="bk-home-primary-modes mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><ModeCard icon={<Trophy/>} label="Fantasy" onClick={()=>onNavigate('fantasy')}/><ModeCard icon={<Target/>} label="Picks" onClick={()=>onNavigate('sportsbook')}/><ModeCard icon={<Brain/>} label="Trivia" onClick={()=>onNavigate('challenges')}/><ModeCard icon={<Newspaper/>} label="NFL News" onClick={()=>onNavigate('news')}/></section>
    <section className="bk-home-featured" aria-labelledby="home-featured-heading">
      <h3 id="home-featured-heading" className="bk-home-section-title">Featured</h3>
      <button type="button" onClick={onOpenCheatSheet}>
        <span className="bk-home-featured-icon" aria-hidden="true"><ClipboardList size={24}/></span>
        <span className="bk-home-featured-copy"><strong>Your 2026 Fantasy Cheat Sheet</strong><small>Rankings, projections &amp; player research.</small></span>
        <ArrowRight size={16} className="bk-home-featured-arrow"/>
      </button>
    </section>
    <section className="bk-home-league-picker relative">
    ...
    <section className="bk-home-rating mt-3 rounded-2xl border border-white/10 bg-[linear-gradient(110deg,rgba(14,19,26,.97),rgba(6,9,14,.94))] px-4 py-3.5 shadow-[inset_0_1px_rgba(255,255,255,.055),0_16px_45px_rgba(0,0,0,.22)] sm:px-5">
    ...
    <section className="bk-home-current-league relative mt-3 min-h-[258px] overflow-hidden rounded-[1.4rem] border border-[rgb(var(--bk-team-primary-rgb)/.60)] bg-[#071014] p-5 shadow-[inset_0_1px_rgba(255,255,255,.08),0_24px_70px_rgba(0,0,0,.38)] sm:p-7">
