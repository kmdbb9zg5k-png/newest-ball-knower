...
const Action=({icon,label,onClick}:{icon:React.ReactNode;label:string;onClick:()=>void})=><button onClick={onClick} className="flex min-h-[70px] min-w-0 flex-col items-center justify-center gap-1.5 border-r border-white/10 px-1 text-center text-[7px] font-black uppercase tracking-[.05em] text-zinc-300 last:border-r-0 sm:min-h-[76px] sm:text-[9px]"><span className="text-[#D9B43B] [&>svg]:h-5 [&>svg]:w-5">{icon}</span><span className="whitespace-nowrap">{label}</span></button>;
