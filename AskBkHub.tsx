      ...
      <div className="flex min-h-[56dvh] flex-col bg-[#080b10]/90">
        <Conversation className="min-h-0 flex-1">
          <ConversationContent className="mx-auto w-full max-w-3xl px-3 py-5 sm:px-5">
            {messages.length===0&&<div className="flex min-h-[30dvh] flex-col items-center justify-center px-2 text-center"><div className="grid h-14 w-14 place-items-center rounded-2xl border border-white/10 bg-white/[.035]"><ShieldCheck className="h-6 w-6 text-[var(--bk-team-accent)]"/></div><h3 className="mt-4 font-display text-2xl font-black uppercase">What do you need to know?</h3><p className="mt-1 max-w-md text-xs leading-5 text-zinc-500">Current questions can use live search. Fantasy advice can also use your active Ball Knower league and roster.</p><div className="mt-5 grid w-full gap-2 sm:grid-cols-2">{suggestions.map(suggestion=><button key={suggestion} type="button" onClick={()=>setDraft(suggestion)} className="min-h-12 rounded-xl border border-white/10 bg-white/[.025] px-3 text-left text-[11px] font-bold text-zinc-300 hover:border-[var(--bk-team-accent)]/30 hover:bg-[var(--bk-team-accent)]/[.06]">{suggestion}</button>)}</div></div>}
