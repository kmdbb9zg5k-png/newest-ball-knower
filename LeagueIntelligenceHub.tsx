...
export const LeagueIntelligenceHub:React.FC<{league:League}>=({league})=>{
 ...
 return <section className="space-y-5">
  ...
  {tab==='rankings'&&<div className="space-y-3"><Header title="Weekly Power Rankings" sub="Record, roster strength, efficiency, balance and point differential combined"/>{rankings.map(r=><div key={r.memberId} className="grid grid-cols-[52px_1fr_auto] items-center gap-3 rounded-2xl border border-white/10 bg-[#101318] p-4"><div className="text-center"><div className="text-2xl font-black text-[#D4AF37]">#{r.rank}</div><div className={`text-[9px] font-black ${r.movement>0?'text-emerald-400':r.movement<0?'text-red-400':'text-zinc-600'}`}>{r.movement>0?`▲${r.movement}`:r.movement<0?`▼${Math.abs(r.movement)}`:'—'}</div></div><div><div className="font-black uppercase">{r.memberName}</div><p className="mt-1 text-[11px] text-zinc-500">{r.reason}</p></div><div className="text-right"><div className="text-xl font-black">{r.score}</div><div className="text-[9px] text-zinc-600">POWER</div></div></div>)}</div>}
