...
const MockWorkspace=({league}:{league:League})=>{const {currentUser,updateLeagueSettings,showToast}=useBallKnower();const [seed,setSeed]=useState(1);const picks=useMemo(()=>buildMockDraft(league,seed),[league,seed]);const canConvert=isLeagueCommissioner(league,currentUser?.id);return <section className="mt-4 rounded-2xl border border-sky-300/25 bg-black/25 p-4"><div className="flex items-center gap-2 text-xs font-black uppercase text-sky-300"><RefreshCw className="h-4 w-4"/>Practice Mock Draft</div><p className="mt-2 text-[10px] leading-4 text-zinc-500">A private practice simulation. It never writes rosters, standings, transactions, or league history. Rerun it to rehearse different boards.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><button onClick={()=>setSeed(value=>value+1)} className="min-h-11 rounded-xl bg-sky-300 px-4 text-[10px] font-black uppercase text-black">Run Another Mock</button>{canConvert&&<button onClick={()=>{updateLeagueSettings(league.id,{draftFormat:'live_snake'});showToast('Mock complete. Live Snake draft is now enabled.');}} className="min-h-11 rounded-xl border border-[#D4AF37]/40 px-4 text-[10px] font-black uppercase text-[#D4AF37]">Continue With Live Draft</button>}</div><div className="mt-3 max-h-72 space-y-1 overflow-y-auto">{picks.map((pick,index)=>{const member=league.members.find(item=>item.id===pick.memberId);const player=PLAYERS_DATABASE.find(item=>item.id===pick.playerId);return <div key={`${index}:${pick.playerId}`} className="flex items-center rounded-lg bg-black/30 px-3 py-2 text-[10px]"><b className="w-8 text-[#D4AF37]">#{index+1}</b><span className="min-w-0 flex-1 truncate">{member?.userName}</span><span className="truncate text-zinc-400">{player?.position} {player?.name}</span></div>})}</div></section>;};
