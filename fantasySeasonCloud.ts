...
export async function sendFantasyDm(threadId:string,body:string){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.rpc('send_ball_knower_dm',{p_thread_id:threadId,p_body:body});if(error)throw error;}
export async function markFantasyDmRead(threadId:string){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.rpc('mark_ball_knower_dm_read',{p_thread_id:threadId});if(error)throw error;}
export async function sendTradeThreadMessage(tradeId:string,body:string){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.rpc('send_ball_knower_trade_message',{p_trade_id:tradeId,p_body:body});if(error)throw error;}
export async function markTradeThreadRead(tradeId:string){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.rpc('mark_ball_knower_trade_thread_read',{p_trade_id:tradeId});if(error)throw error;}
...
export async function setTradingBlockEntry(leagueId:string,memberId:string,playerId:string,status:TradingBlockEntry['status'],lookingFor:string[]=[],note=''){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.from('ball_knower_trading_block').upsert({league_id:leagueId,member_id:memberId,player_id:playerId,status,looking_for:lookingFor,note:note||null,updated_at:new Date().toISOString()},{onConflict:'league_id,member_id,player_id'});if(error)throw error;}
export async function removeTradingBlockEntry(leagueId:string,memberId:string,playerId:string){if(!supabase)return;await ensureOnlineSession();const {error}=await supabase.from('ball_knower_trading_block').delete().eq('league_id',leagueId).eq('member_id',memberId).eq('player_id',playerId);if(error)throw error;}
export async function setWatchedFantasyPlayer(leagueId:string,playerId:string,watched:boolean){if(!supabase)return;const auth=await ensureOnlineSession();const query=supabase.from('ball_knower_watched_players');const {error}=watched?await query.upsert({league_id:leagueId,auth_user_id:auth.id,player_id:playerId},{onConflict:'league_id,auth_user_id,player_id'}):await query.delete().eq('league_id',leagueId).eq('auth_user_id',auth.id).eq('player_id',playerId);if(error)throw error;}
