...
export const SoloMode:React.FC=()=>{
 ...
 return <div className="min-h-[100dvh] bg-transparent text-white px-4 sm:px-8 py-8"><div className="mx-auto max-w-7xl">
  <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-8"><div><div className="text-[var(--bk-team-accent)] text-xs font-black tracking-[.3em]">SOLO MODE</div><h2 className="text-4xl sm:text-6xl font-black">ROAD TO THE <span className="text-[var(--bk-team-accent)]">SUPER BOWL</span></h2><p className="text-zinc-400 mt-2">Draft. Survive 17 weeks. Chase a seed. Win four playoff games. Build your legacy.</p></div><div className="flex gap-2"><button onClick={share} className="flex gap-2 items-center border border-white/10 px-4 py-2 bg-[#151515]"><Share2 size={16}/> Share</button><button onClick={reset} className="flex gap-2 items-center border border-white/10 px-4 py-2 bg-[#151515]"><RotateCcw size={16}/> New Run</button></div></div>
...
const PlayerPhoto=({player}:{player:Player})=>{const portrait=playerPortraitUrl(player);const initials=player.name.split(' ').map(part=>part[0]).slice(0,2).join('');return <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-full border border-white/10 bg-[#222]"><span className="absolute inset-0 grid place-items-center text-xs font-black text-zinc-500">{initials}</span>{portrait&&<img src={portrait} alt={player.name} loading="lazy" decoding="async" referrerPolicy="no-referrer" onError={event=>{event.currentTarget.style.display='none'}} className="relative h-full w-full object-cover"/>}</div>};
