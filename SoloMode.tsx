import {SoloPlayerLink,SoloPortrait,useSoloRecords,withSoloPresentation} from './solo/SoloPresentation';
...
export const SoloMode:React.FC<{initialExperience?:SoloExperience}>=withSoloPresentation<{initialExperience?:SoloExperience}>(({initialExperience='hub'})=>{
...
const CapChallenge:React.FC<{onBack:()=>void}>=({onBack})=>{
 ...
 useSoloRecords(roster,weeks,interactions);
 ...
 const leaders=useMemo(()=>{
   const m=new Map<string,{id:string;name:string;pos:string;score:number}>();
   allLines.forEach(l=>{const x=m.get(l.playerId)||{id:l.playerId,name:l.name,pos:l.position,score:0};x.score+=l.fantasyScore;m.set(l.playerId,x)});
 ...
 return <BroadcastStage scene="tunnel" page="cap" quiet={true} className="min-h-[100dvh] bg-transparent text-white px-4 sm:px-8 pt-4 pb-8"><div className="mx-auto max-w-7xl">
  ...
  {stage==='regular'&&<><FranchiseInteractionCenter state={interactions} roster={roster} onRespond={respondToScenario} onUpgrade={applyUpgrade} onReadAll={readAllNotifications}/><div className="grid lg:grid-cols-[1.4fr_.7fr] gap-6"><div><div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5"><Stat label="Week" value={`${weeks.length+1>17?'17':weeks.length+1}/17`}/><Stat label="Record" value={`${wins}-${losses}`}/><Stat label={weeks.length===17?'Playoff Status':'Projected Seed'} value={weeks.length===17?(finalPlayoffOdds===100?'CLINCHED':'OUT'):`#${weeks.at(-1)?.playoffSeed||'—'}`}/><Stat label="Playoff Odds" value={`${finalPlayoffOdds}%`}/><Stat label="Team OVR" value={`${ratings.overall}`}/></div>
   ...
   <aside className="space-y-5"><Panel title="INJURY REPORT" icon={<ShieldAlert size={18}/>}>{activeInjuries.length?activeInjuries.map(i=>{const player=PLAYER_BY_ID.get(i.playerId);return <div key={i.playerId} className="border-b border-white/5 py-2"><b>{player?<SoloPlayerLink player={player}/>:i.playerName}</b><div className="text-xs text-zinc-500">{i.position} • {i.weeks} week(s) remaining • {i.severity}</div></div>}):<p className="text-zinc-500 text-sm">Healthy roster.</p>}</Panel><Panel title="TEAM LEADERS" icon={<BarChart3 size={18}/>}>{leaders.map((l,i)=>{const player=PLAYER_BY_ID.get(l.id);return <div key={l.id} className="flex justify-between py-2 border-b border-white/5"><span>{i+1}. {player?<SoloPlayerLink player={player}/>:l.name} <small className="text-zinc-500">{l.pos}</small></span><b>{l.score.toFixed(1)}</b></div>})}</Panel></aside></div></>}
  ...
  {stage==='finished'&&<div className="max-w-5xl mx-auto"><div className="text-center border border-[var(--bk-team-accent)]/40 bg-[#111] p-8"><Crown className="mx-auto text-[var(--bk-team-accent)]" size={70}/><h3 className="text-5xl font-black mt-3">{message.includes('WORLD CHAMPION')?'LEGACY BOWL CHAMPION':'RUN COMPLETE'}</h3><p className="text-xl text-zinc-300 mt-3">{message}</p><div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6"><Stat label="Record" value={`${wins}-${losses}`}/><Stat label="Team OVR" value={`${ratings.overall}`}/><Stat label="Draft Grade" value={grade.letter}/><Stat label="BK Score" value={`${grade.score}`}/></div></div>
   <div className="grid md:grid-cols-2 gap-6 mt-6"><Panel title="SEASON AWARDS" icon={<Award size={18}/>}>{awards.map(a=>{const player=roster.find(candidate=>candidate.name===a.winner);return <div key={a.award} className="py-3 border-b border-white/5"><div className="text-[10px] text-[var(--bk-team-accent)] font-black tracking-wider">{a.award}</div><div className="text-lg font-black">{player?<SoloPlayerLink player={player}/>:a.winner}</div></div>})}</Panel><Panel title="ACHIEVEMENTS UNLOCKED" icon={<Trophy size={18}/>}>{achievementsForRun(wins,losses,message.includes('WORLD CHAMPION'),grade.score,roster).map(a=><div key={a} className="py-2 font-black">🏆 {a}</div>)}</Panel></div>
...
const PlayerPhoto=({player}:{player:Player})=><SoloPortrait player={player} className="bk-solo-cap-portrait"/>;
