...
const restoreRun=()=>{
 try{
   ...
   return {...saved,weeks,playoffs,settings:{difficulty,injuries:injurySetting}};
 }catch(error){
...
export const SoloMode:React.FC=()=>{
 ...
 return <div className="min-h-[100dvh] bg-transparent text-white px-4 sm:px-8 pt-4 pb-8"><div className="mx-auto max-w-7xl">
  ...
  <details className="mt-3 border border-white/10 bg-[#101010]/90"><summary className="flex min-h-12 cursor-pointer list-none items-center justify-between px-4 text-sm font-black"><span>CAREER STATS</span><span className="text-xs text-zinc-500">{career.runs} RUNS • {career.championships} TITLES</span></summary><div className="grid grid-cols-2 md:grid-cols-6 gap-2 border-t border-white/10 p-3"><Stat label="Career Runs" value={`${career.runs}`}/><Stat label="Titles" value={`${career.championships}`}/><Stat label="Career W-L" value={`${career.regularWins}-${career.regularLosses}`}/><Stat label="Playoff Wins" value={`${career.playoffWins}`}/><Stat label="Best Record" value={career.bestRecord}/><Stat label="Best BK Score" value={`${career.bestScore}`}/></div></details>
 </div></div>
};
