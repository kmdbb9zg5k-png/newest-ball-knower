...
var STAFF_ART_BY_ID = {
  "solo-staff-owner-amara-okafor": { role: "owner", name: "Amara Okafor", nationality: "Nigerian", visualDirection: "Nigerian woman in her late forties with deep brown skin, a poised oval face, natural textured updo and understated executive styling" },
  "solo-staff-owner-kenji-watanabe": { role: "owner", name: "Kenji Watanabe", nationality: "Japanese", visualDirection: "Japanese man in his early fifties with warm light skin, short salt-and-pepper hair, a composed angular face and refined executive styling" },
  "solo-staff-owner-valentina-rojas": { role: "owner", name: "Valentina Rojas", nationality: "Colombian", visualDirection: "Colombian woman in her early forties with medium olive skin, long dark waves, strong brows and confident modern executive styling" },
...
function fictionalUniformPrompt(player, number, variant = "home") {
  ...
  const componentLock = variant === "home"
    ? `jersey, pants, helmet and socks must all use primary ${primary}; secondary ${secondary} is permitted only for number ${number}, thin trim and narrow stripes—never for the pants, helmet shell or large shoulder panels`
    : `jersey base ${jerseyBase}; pants, helmet and socks use primary ${primary}; number ${number} uses ${numberColor}; secondary ${secondary} is trim only`;
  return `Original fictional professional football ${variant} uniform. STRICT TEAM PALETTE LOCK: ${componentLock}. The chest-up and full-body panels must show the identical uniform design, colors, number ${number}, striping and equipment. Do not invent or substitute any color outside this palette. Every garment and helmet surface must be completely unbranded: no NFL, real-team, league, sponsor, wordmark, mascot, manufacturer, checkmark, wing, swoosh or logo-like marks anywhere`;
}
...
async function reusableTeamUniformAnchor(service, job) {
  ...
  // A player photo is too strong a conditioning signal: it can clone that
  // player's face and body across the roster. This faceless palette board gives
  // Gemini the exact uniform colors and trim hierarchy without an identity to copy.
