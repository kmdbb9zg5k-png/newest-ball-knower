    ...
    <section className="space-y-4 pb-24 text-white">
      ...
      {tab === 'activity' && <div className="space-y-3">
        ...
        {activityView === 'trades' && <div className="space-y-3">
          ...
          {reviewTrades.length > 0 && settings.tradeReview==='league_vote' && <Panel title="League Vote" sub="Trade participants cannot vote; a majority of other human managers decides" icon={<Gavel className="h-5 w-5 text-[#D4AF37]"/>}>{reviewTrades.map(trade => <div key={trade.id} className="space-y-2 rounded-xl bg-black/25 p-3"><TradeSummary trade={trade} proposer={league.members.find(member => member.id === trade.proposerMemberId)} recipient={league.members.find(member => member.id === trade.recipientMemberId)} findPlayer={findPlayer} valueLabel={valueLabel}/><div className="grid grid-cols-2 gap-2"><button disabled={busy} onClick={()=>run(async()=>{const result=await voteOnFantasyTrade(trade.id,'approve');showToast(result.status==='accepted'?'Trade approved and completed.':`Approval saved · ${result.approvals}/${result.needed}`);})} className="min-h-10 rounded-lg bg-emerald-500/15 text-[9px] font-black uppercase text-emerald-300">Approve</button><button disabled={busy} onClick={()=>run(async()=>{const result=await voteOnFantasyTrade(trade.id,'veto');showToast(result.status==='vetoed'?'Trade vetoed.':`Veto saved · ${result.vetoes}/${result.needed}`);})} className="min-h-10 rounded-lg bg-red-500/10 text-[9px] font-black uppercase text-red-300">Veto</button></div></div>)}</Panel>}
