  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3">
    ...
    {open&&<div className="space-y-3 border-t border-white/10 pt-3">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <Select label="Regular Season" value={String(effectiveRegularSeasonWeeks)} disabled={disabled||scheduleLocked} options={[13,14,15,16,17].map(value=>option(String(value),`${value} Weeks`))} onChange={value=>update({regularSeasonWeeks:Number(value) as LeagueSettings['regularSeasonWeeks']})}/>
        <Select label="Playoffs" value={String(settings.playoffTeams||6)} disabled={disabled||postseasonLocked} options={[4,6,8].filter(value=>value<=league.maxMembers).map(value=>option(String(value),`${value} Teams`))} onChange={value=>update({playoffTeams:Number(value) as LeagueSettings['playoffTeams']})}/>
