  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3">
    ...
    {open&&<div className="space-y-3 border-t border-white/10 pt-3">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <Select label="Divisions" value={settings.divisionsEnabled?String(settings.divisionCount||2):'off'} disabled={disabled} options={[option('off','Off'),option('2','2 Divisions'),option('4','4 Divisions')]} onChange={value=>update(value==='off'?{divisionsEnabled:false}:{divisionsEnabled:true,divisionCount:Number(value) as 2|4})}/>
      </div>
      <div className="rounded-xl border border-white/10 p-3"><div className="text-[9px] font-black uppercase tracking-wider text-zinc-500">Custom scoring values</div><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4"><ScoreInput label="Reception" value={settings.customScoring?.reception??(settings.scoringFormat==='standard'?0:settings.scoringFormat==='half_ppr'?0.5:1)} disabled={disabled} onCommit={value=>update({customScoring:{...settings.customScoring,reception:value}})}/><ScoreInput label="Pass TD" value={settings.customScoring?.passTd??4} disabled={disabled} onCommit={value=>update({customScoring:{...settings.customScoring,passTd:value}})}/><ScoreInput label="Rush TD" value={settings.customScoring?.rushTd??6} disabled={disabled} onCommit={value=>update({customScoring:{...settings.customScoring,rushTd:value}})}/><ScoreInput label="Rec TD" value={settings.customScoring?.recTd??6} disabled={disabled} onCommit={value=>update({customScoring:{...settings.customScoring,recTd:value}})}/></div></div>
