...
export const FranchiseSeason: React.FC<Props> = ({
  ...
  const rosterLegal = roster.length === TOTAL_ROSTER_SIZE && rosterNeeds.length === 0 && capUsed <= DEFAULT_SALARY_CAP + 0.001;
  ...
  const playWeek = () => {
    ...
    if (!rosterLegal) {
      setMessage(`Roster management required before Week ${weekNumber}. Return to Franchise Command and get to ${TOTAL_ROSTER_SIZE} legal players under the salary cap.`);
      return;
    }
  ...
  const selectProspect = (prospect: RookieProspect) => {
    ...
    if (!nextRound) {
      ...
      setDraftRound(1);
      setDraftClassNumber(value => value + 1);
      setMessage(`Draft complete — ${next.map(player => player.name).join(', ')} joined your franchise. Return to Franchise Command and cut back to a legal ${TOTAL_ROSTER_SIZE}-player roster before Week 1.`);
      onDraftComplete?.();
    } else {
