...
import { LeagueMember, Player, Position } from './types';
...
function normalizeDraftedProspects(value: unknown): DraftedProspect[] {
  ...
  return value.flatMap((item, index) => {
    ...
    return [{
      ...
      grade: Number(prospect.grade),
      round: ALL_DRAFT_ROUNDS.includes(round) ? round : Math.min(7, index + 1),
    }];
  });
}
...
const OffseasonDraft = ({ round, wins, selected, ownedRounds, onSelect }: { round: number; wins: number; selected: DraftedProspect[]; ownedRounds: number[]; onSelect: (prospect: RookieProspect) => void }) => {
