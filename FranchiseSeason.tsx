...
export const FranchiseSeason: React.FC<Props> = ({
  ...
  const restoredYear = Number.isInteger(restored?.year) && restored.year >= 2026 ? restored.year : 2026;
  const restoredClassIds = new Set(buildFranchiseRookieClass(restoredYear + 1).map(prospect => prospect.id));
  const restoredDraftIsCurrent = Array.isArray(restored?.draftedProspects)
    && restored.draftedProspects.every((prospect: RookieProspect) => restoredClassIds.has(prospect.id));
...
const rookieArtPlayer = (prospect:RookieProspect, year:number) => ({
  id:`franchise-rookie-${year}-${prospect.id}`,
  name:prospect.name,
  position:prospect.position,
  team:'FA',
  teamName:'Ball Knower Draft Class',
  age:22,
});
...
const OffseasonDraft = ({ year, round, wins, selected, ownedRounds, onSelect }: { year: number; round: number; wins: number; selected: RookieProspect[]; ownedRounds: number[]; onSelect: (prospect: RookieProspect) => void }) => {
  ...
  return <div className="grid gap-4 lg:grid-cols-[1fr_19rem]"><section className="rounded-[2rem] border border-[var(--bk-team-accent)]/25 bg-[#10151d] p-5 sm:p-7"><div className="flex items-center gap-3 text-[var(--bk-team-accent)]"><Users/><span className="text-[10px] font-black tracking-[.24em]">LIVE 32-TEAM ROOKIE DRAFT</span></div><h2 className="mt-2 text-4xl font-black">YOU'RE ON THE CLOCK</h2><p className="mt-2 text-sm text-zinc-400">Round {round}, Pick {draftSlot}. The {cpuBefore} teams ahead of you have already simulated their selections. Your owned rounds: {ownedRounds.join(', ')}.</p><div className="mt-5 grid gap-2 sm:grid-cols-2">{available.map(prospect => <button key={prospect.id} onClick={() => onSelect(prospect)} className="flex min-h-20 items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 p-3 text-left hover:border-[var(--bk-team-accent)]/50"><SoloPortrait player={rookieArtPlayer(prospect,year)}/><div className="min-w-0 flex-1"><div className="truncate text-base font-black">{prospect.name}</div><div className="text-[10px] font-bold text-zinc-500">{prospect.position} · {prospect.school}</div></div><div className="text-right"><div className="text-xl font-black text-[var(--bk-team-accent)]">{prospect.grade}</div><div className="text-[8px] font-black text-zinc-600">SCOUT GRADE</div></div></button>)}</div></section><aside className="space-y-3"><SeasonPanel title="YOUR DRAFT CLASS">{selected.length ? selected.map((prospect, index) => <div key={prospect.id} className="flex items-center gap-2 border-b border-white/5 py-2 text-sm"><SoloPortrait player={rookieArtPlayer(prospect,year)} size="avatar"/><div><b>R{ownedRounds[index]}: {prospect.name}</b><div className="text-xs text-zinc-500">{prospect.position} · {prospect.school}</div></div></div>) : <p className="text-sm text-zinc-500">No picks yet.</p>}</SeasonPanel><SeasonPanel title="CPU PICK SIMULATION"><p className="text-sm text-zinc-400">Rounds you traded away are owned and used by the acquiring team. The board advances only to selections your franchise still owns.</p></SeasonPanel></aside></div>;
};
