...
const ranksFor = (snapshots: Snapshot[], key: string) => {
  ...
  ordered.forEach((snapshot, index) => {
    ...
    if (rank !== previousRank || previousValue === null) previousValue = value;
    previousRank = rank;
  });
  return ranks;
};
...
export const buildFantasyDraftReports = (
  ...
  const games = Number.isFinite(regularSeasonGames) ? clamp(Math.round(regularSeasonGames), 1, 18) : 14;
  const values = snapshots.map(snapshot => snapshot.strength);
  const mean = values.reduce((sum, value) => sum + value, 0) / values.length;
  const deviation = Math.sqrt(values.reduce((sum, value) => sum + (value - mean) ** 2, 0) / values.length);
  const meaningfulDeviation = Math.max(MIN_MEANINGFUL_ROSTER_SPREAD, Math.abs(mean) * 0.01, deviation);
  ...
  snapshots.forEach((snapshot, index) => {
    const projectionScore = clamp(Math.round(
      86 + ((snapshot.strength - mean) / meaningfulDeviation) * 6.5,
    ), 68, 98);
