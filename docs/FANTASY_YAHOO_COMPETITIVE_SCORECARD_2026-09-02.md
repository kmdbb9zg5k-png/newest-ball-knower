...
| 24 | Notifications | Current mobile listing advertises customizable game-start, scoring, roster-news and trade alerts. | In-app event persistence plus draft, trade, waiver, matchup and lineup notification generation/receipts. | **Yahoo still better** | Native device-token registration, push delivery, preference controls, and physical-device validation remain incomplete. |
...
5. Native push transport still requires app-shell device-token registration and physical delivery verification.
...
1. Finish Phase A with real-device testing, notification transport/preferences, direct authorization integration tests, and production monitoring through live NFL traffic.
