...
export const MyPlayerStory: React.FC<Props> = ({ onBack }) => {
  ...
  const onFace = async (file?: File) => {
    ...
    try {
      ...
      setMessage('Face uploaded. Describe the player look you want.');
    } catch {
  ...
  const createRender = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/my-player-art', {
        ...
        body: JSON.stringify({ image: profile.faceImage, prompt: profile.appearancePrompt, position: profile.position, number: profile.number, team: 'a future pro team' }),
      });
  ...
  const enterDraft = () => {
    const value = hash(`${profile.name}:${profile.position}:${profile.number}`);
    ...
    <div className="min-h-[100dvh] bg-transparent px-4 pb-10 pt-4 text-white sm:px-8">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center"><div className="text-[10px] font-black tracking-[.22em] text-[var(--bk-team-accent)]">MY PLAYER STORY</div><div className="text-sm font-black">{profile.stage === 'creator' ? 'CREATE' : profile.stage === 'combine' ? 'NFL COMBINE' : 'DRAFT NIGHT'}</div></div>
        ...
        {profile.stage === 'creator' ? (
          <div className="mt-5 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-[#10151d] p-5 sm:p-7">
              ...
              <p className="mt-2 text-sm font-semibold text-zinc-400">Upload a clear selfie, choose your position and describe the game-day look you want.</p>
              ...
              <div className="mt-4"><div className="text-[10px] font-black text-zinc-500">POSITION</div><div className="mt-2 flex flex-wrap gap-2">{POSITIONS.map(position => <button key={position} type="button" aria-pressed={profile.position===position} onClick={() => setProfile(current => ({ ...current, position }))} className={`min-h-10 rounded-xl border px-3 text-xs font-black ${profile.position === position ? 'border-[var(--bk-team-accent)] bg-[var(--bk-team-accent)]/10 text-[var(--bk-team-accent)]' : 'border-white/10 bg-[#151515] text-zinc-400'}`}>{position}</button>)}</div></div>
              <label className="mt-4 block text-[10px] font-black text-zinc-500">DESCRIBE YOUR LOOK<textarea value={profile.appearancePrompt} onChange={event => setProfile(current => ({ ...current, appearancePrompt: event.target.value.slice(0, 280) }))} className="mt-1 min-h-24 w-full rounded-xl border border-white/10 bg-[#151515] p-3 text-sm text-white outline-none" placeholder="Add a tattoo sleeve, dark visor, white gloves…" /></label>
              ...
              {aiAvailable === false ? <p className="mt-2 text-center text-[10px] font-bold text-zinc-500">The built-in player preview and full career still work while the photorealistic image service is offline.</p> : null}
        ...
        {profile.stage === 'combine' ? <Combine profile={profile} onDraft={enterDraft} /> : null}

        {profile.stage === 'drafted' && draftedTeam ? (
          <div className="mt-6 rounded-[2rem] border border-white/10 bg-[#10151d] p-7 text-center">
            ...
            <p className="mt-2 text-zinc-400">select {profile.name}, {profile.position}. Your NFL story starts now.</p>
...
const PlayerRender = ({ profile }: { profile: MyPlayerProfile }) => {
  ...
  if (profile.renderImage) return <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#111]"><img src={profile.renderImage} alt={`${profile.name || 'Created player'} render`} className="aspect-[4/5] h-full w-full object-cover" /></div>;
  return (
    <div className="relative min-h-[500px] overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_50%_20%,rgba(90,120,180,.35),transparent_32%),linear-gradient(160deg,#111827,#05070b)]">
      <div className="absolute inset-x-0 top-10 text-center text-[10px] font-black tracking-[.28em] text-white/40">BALL KNOWER PLAYER RENDER</div>
      <div className="absolute inset-x-[12%] bottom-8 top-24 rounded-[45%_45%_16%_16%] bg-gradient-to-b from-zinc-500 via-zinc-800 to-black shadow-2xl">
        <div className="absolute left-1/2 top-3 h-28 w-24 -translate-x-1/2 overflow-hidden rounded-[45%] border-4 border-zinc-400 bg-zinc-900 shadow-xl">{profile.faceImage ? <img src={profile.faceImage} alt="Your uploaded face" className="h-full w-full object-cover" /> : <Camera className="m-auto mt-8 text-zinc-600" size={34} />}</div>
        <div className="absolute left-1/2 top-32 -translate-x-1/2 text-7xl font-black text-white/90">{profile.number}</div>
        <div className="absolute inset-x-3 bottom-5 flex flex-wrap justify-center gap-1">{tags.map(tag => <span key={tag} className="rounded-full bg-black/55 px-2 py-1 text-[8px] font-black text-white/70">{tag}</span>)}</div>
      </div>
    </div>
  );
};
...
const Combine = ({ profile, onDraft }: { profile: MyPlayerProfile; onDraft: () => void }) => {
  const base = hash(`${profile.name}:${profile.position}`);
  const forty = (4.32 + (base % 48) / 100).toFixed(2);
  const bench = 12 + ((base >>> 4) % 20);
  const vertical = 31 + ((base >>> 7) % 12);
  return (
    <div className="mt-6 rounded-[2rem] border border-white/10 bg-[#10151d] p-6 sm:p-8">
