...
import {Bandage,Clock3,Gavel,RefreshCw,ShieldAlert} from 'lucide-react';
import {League} from './types';
import {useBallKnower} from './BallKnowerContext';
import {fetchNextInjuryWeek,fetchSeasonOperations,generateWeeklyInjuries,WaiverClaim} from './fantasySeasonCloud';
...
export const FantasySeasonAdmin:React.FC<{league:League}>=({league})=>{
  ...
  return <section className="relative mt-5 overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#070a0f] p-4 shadow-[0_28px_70px_rgba(0,0,0,.48)] sm:p-5">
    ...
    <div className="relative z-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.16em] text-[var(--bk-team-accent)]"><Gavel className="h-4 w-4"/>Season Commissioner Tools</div><p className="mt-1 max-w-xl text-[11px] font-semibold text-zinc-400">Waivers process automatically; injury reports remain commissioner-controlled and retry-safe.</p></div><button onClick={()=>void refresh()} aria-label="Refresh commissioner tools" className="flex min-h-11 min-w-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-black/35 px-3 text-[10px] font-black uppercase text-zinc-300 shadow-lg backdrop-blur-md transition hover:border-white/20 hover:bg-white/5"><RefreshCw className="h-3.5 w-3.5"/>Refresh</button></div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-black/35 p-4 shadow-[0_16px_40px_rgba(0,0,0,.28)] backdrop-blur-md"><div className="flex items-center gap-2 text-xs font-black uppercase"><Clock3 className="h-4 w-4 text-emerald-400"/>Automatic Waivers</div><div className="mt-2 text-[11px] leading-relaxed text-zinc-400">{pending.length} pending claim{pending.length===1?'':'s'} · the secure processor checks the queue every minute and executes claims at the league's scheduled hour.</div><div className="mt-3 rounded-xl border border-emerald-500/15 bg-emerald-500/10 px-3 py-2 text-[10px] font-black uppercase text-emerald-300">No commissioner action needed</div></div>
