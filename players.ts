...
export interface NFLTeamInfo {
  code:string; name:string; city:string; conference:'AFC'|'NFC'; division:'East'|'North'|'South'|'West';
...
export function getPositionGroup(pos:Position):PositionGroup{
 ...
 if(['OT','LT','RT','OG','LG','RG','C','OL'].includes(p))return 'OL' as PositionGroup;
...
function normalizePlayer(raw:Player):Player{
 ...
 const madden=getOfficialMaddenRating(raw.id,raw.name,raw.team,raw.position);
 const ovr=(raw as any).overallRating || (madden as any)?.overallRating || raw.ovr;
 return {...raw,
  playerId:(raw as any).playerId||raw.id,teamId:(raw as any).teamId||raw.team,teamAbbreviation:raw.team,
  teamCity:(raw as any).teamCity||teamData?.city||'NFL',teamName:(raw as any).teamName||teamData?.name||'',
  conference:(raw as any).conference||teamData?.conference||'AFC',division:(raw as any).division||teamData?.division||'East',
  positionGroup:(raw as any).positionGroup||getPositionGroup(raw.position),active:(raw as any).active!==false,
  rosterSeason:2026,overallRating:ovr,ovr,overall:ovr,
 } as Player;
}
